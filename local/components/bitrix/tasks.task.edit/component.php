						...
						if(intval($templateId))
						{
							...
							if(is_array($_POST['CHECKLIST_ITEM_ID']))
							{
								...
								if(!empty($items))
								{
									...
									try
									{
										\Bitrix\Tasks\Template\CheckListTable::updateForTemplate($templateId, $items);
									}
			...
			if ($arTemplate = $rsTemplate->GetNext())
			{
				if ($arTemplate["CREATED_BY"] == $loggedInUserId)
				{
					...
					if ($arTemplate["DEADLINE_AFTER"])
					{
						...
						$arTemplate["DEADLINE"] = \Bitrix\Tasks\UI::formatDateTime(strtotime(date("Y-m-d 00:00")." +".$deadlineAfter." days"));
					}

					// check list items
					$res = \Bitrix\Tasks\Template\CheckListTable::getList(array(
