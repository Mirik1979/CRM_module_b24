...
elseif($action == 'GET_ENTITY_COMMUNICATIONS')
{
	...
	$resu=GetCrmEntityCommunications($entityType, $entityID, $communicationType);

	//print_r($resu);
	if($resu["DATA"]["TABS"] && !$useContact){
		for ($i=0;$i<count($resu["DATA"]["TABS"]);$i++)
			if($resu["DATA"]["TABS"][$i]["id"]=='company')
				unset($resu["DATA"]["TABS"][$i]);
		$resu["DATA"]["TABS"] = array_values($resu["DATA"]["TABS"]);

		for ($i=0;$i<count($resu["DATA"]["TABS"]);$i++){
			for ($k=0;$k<count($resu["DATA"]["TABS"][$i]["items"]);$k++)
				if($resu["DATA"]["TABS"][$i]["items"][$k]["entityType"]=='CONTACT')
					unset($resu["DATA"]["TABS"][$i]["items"][$k]);
			$resu["DATA"]["TABS"][$i]["items"] = array_values($resu["DATA"]["TABS"][$i]["items"]);
		}

	}elseif($useContact && $contactid>0){
	...
	__CrmActivityEditorEndResponse($resu);
}
