...
foreach($arResult['ITEMS'] as &$item)
{
	...
	if($isEditable)
	{
		...
		if($item['CAN_COMPLETE']
			&& $itemTypeID !== CCrmActivityType::Email
			&& $itemTypeID !== CCrmActivityType::Call
			&& $itemTypeID !== CCrmActivityType::Meeting) //Email is always COMPLETED
		{
	...
	if($completed === 'Y'):
		...
		//wcomm { 
		$itemTypeID = intval($item['TYPE_ID']);
		if(($itemTypeID === CCrmActivityType::Call) || ($itemTypeID === CCrmActivityType::Meeting))
		{
			$completedOnClick = 'return (false && false);';
		} else {
...
if($enableToolbar)
{
    ...
    if ($USER->isAdmin()==false) {
        unset($toolbarButtons[4]);
        unset($toolbarButtons[5]);
    }
