...
if(typeof BX.Crm.EntityEditorModelFactory === "undefined")
{
	BX.Crm.EntityEditorModelFactory =
	{
		create: function(entityTypeId, id, settings)
		{
			...
			else if(entityTypeId === BX.CrmEntityType.enumeration.construction)
			{
				return BX.Crm.ConstructionModel.create(id, settings);
			}
...
if(typeof BX.Crm.ConstructionModel === "undefined")
{
	BX.Crm.ConstructionModel = function()
	{
		BX.Crm.ConstructionModel.superclass.constructor.apply(this);
	};
	BX.extend(BX.Crm.ConstructionModel, BX.Crm.EntityModel);
	BX.Crm.ConstructionModel.prototype.getEntityTypeId = function()
	{
		return BX.CrmEntityType.enumeration.construction;
	};
	BX.Crm.ConstructionModel.prototype.getCaption = function()
	{
		return this.getField("FORMATTED_NAME", "Создание объекта");
	};
	BX.Crm.ConstructionModel.create = function(id, settings)
	{
		var self = new BX.Crm.ConstructionModel();
		self.initialize(id, settings);
		return self;
	};
}
...
if(typeof BX.Crm.EntityEditorClientSearchBox === "undefined")
{
	...
	BX.Crm.EntityEditorClientSearchBox.prototype =
	{
		...
		prepareMultifieldLayout: function()
		{
			...
			if(this._entityInfo)
			{
				...
				if(phones.length > 0)
				{
					...
					this._maskedPhone.setValue(BX.prop.getString(phone, "VALUE_FORMATTED", ""));
					this._phoneId = BX.prop.getString(phone, "ID", "");
				}
				...
				if(emails.length > 0)
				{
					...
					this._emailInput.value = BX.prop.getString(email, "VALUE_FORMATTED", "");
					this._emailId = BX.prop.getString(email, "ID", "");
				}
			}
			...
			{
				var phoneId = this.parseMultifieldPseudoId(this._phoneId);
				if(phoneId >= 0)
				{
			...
			{
				var emailId = this.parseMultifieldPseudoId(this._emailId);
				if(emailId >= 0)
				{
		...
		parseMultifieldPseudoId: function()
		{
			var m = this._phoneId.match(/^n(\d+)/);
