...
class CrmActivityPlannerComponent extends \CBitrixComponent
{
	...
	private static function getDestinationDealEntities($filter, $limit, $order = array())
	{
        ...
        if($contactid>0){
            $filter=['COMPANY_ID'=>$contactid];
        }

        $filter['!=STAGE_ID'] = ['WON','LOSE','APOLOGY'];
	...
	public static function saveActivity($data, $userID, $siteID)
	{
		...
		$arFields['CALL_FIELDS_DIR'] =  isset($data['CALL_FIELDS_DIR']) ? intval($data['CALL_FIELDS_DIR']) : 0;
		$arFields['CALL_FIELDS_STR'] =  isset($data['CALL_FIELDS_STR']) ? htmlspecialcharsEx(trim($data['CALL_FIELDS_STR'])) : "";

		$arFields['COMPLETED'] = (($arFields['CALL_FIELDS_DIR'] > 0) || ($arFields['CALL_FIELDS_STR'] != "")) ? 'Y' : 'N';
