...
switch ($provider){
    case 'Bitrix\Crm\Activity\Provider\CallList':
        ...
        break;
    case 'Bitrix\Crm\Activity\Provider\Call':
        $provider=\local\Crm\Activity\Provider\CallLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\Custom':
        $provider=\local\Crm\Activity\Provider\CustomLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\Email':
        $provider=\local\Crm\Activity\Provider\EmailLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\ExternalChannel':
        $provider=\local\Crm\Activity\Provider\ExternalChannelLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\Livefeed':
        $provider=\local\Crm\Activity\Provider\LivefeedLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\Meeting':
        $provider=\local\Crm\Activity\Provider\MeetingLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\OpenLine':
        $provider=\local\Crm\Activity\Provider\OpenLineLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\Request':
        $provider=\local\Crm\Activity\Provider\RequestLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\RestApp':
        $provider=\local\Crm\Activity\Provider\RestAppLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\Sms':
        $provider=\local\Crm\Activity\Provider\SmsLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\Task':
        $provider=\local\Crm\Activity\Provider\TaskLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\Visit':
        $provider=\local\Crm\Activity\Provider\VisitLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\Wait':
        $provider=\local\Crm\Activity\Provider\WaitLocal::class;
        break;
    case 'Bitrix\Crm\Activity\Provider\WebForm':
        $provider=\local\Crm\Activity\Provider\WebFormLocal::class;
        break;
}
...
if($arResult['COMMUNICATIONS_DATA'][0]['entityType']=="CONTACT"){
    ...
    if ( \Bitrix\Main\Loader::IncludeModule('crm')) {
        $dbRes = \CCrmContact::GetListEx(array(), array('=ID' => $arResult['COMMUNICATIONS_DATA'][0]['entityId'], 'CHECK_PERMISSIONS' => 'N'), false, false);
        $fields = is_object($dbRes) ? $dbRes->Fetch() : null;
        if (is_array($fields)) {
            $Company=\CCrmCompany::GetByID($fields["COMPANY_ID"],false);
            if($Company["ID"]){
                $communicationsData = htmlspecialcharsbx(\Bitrix\Main\Web\Json::encode([[
                    'id' =>'',
                    'type' =>'',
                    ...
                    'entityUrl' => "/crm/company/details/{$Company["ID"]}/",
                ]]));
            }
        }
    }
...
if(count($list)==1){
     ...
     */
    $el=$list[0];
    $communicationsData2Res=[[
        'entityDescription'=> $el->getDescription(),
        'entityId'=> $el->getContactId(),
        'entityTitle'=> $el->getTitle(),
        'entityType'=> "CONTACT",
        'type'=> "",
        'value'=> "",
    ]];
    ...
    if($arResult['COMMUNICATIONS_DATA'][0]['entityType']=="CONTACT")
        $communicationsData2Res=$arResult['COMMUNICATIONS_DATA'];
    else
        $communicationsData2Res=[];
}
...
$communicationsData2 = htmlspecialcharsbx(\Bitrix\Main\Web\Json::encode($communicationsData2Res));

$APPLICATION->restartBuffer();

\CJSCore::init(array('crm_activity_planner','jquery'));
...
?><!DOCTYPE html>
<html>
<head><? $APPLICATION->showHead(); ?><script><?include_once "planner.js";?></script></head>
				...
				<div class="crm-activity-planner-slider-header-control-block">
					...
					<div class="crm-activity-planner-slider-header-control-item crm-activity-planner-slider-header-control-important crm-activity-planner-slider-header-icon-flame-active">
						<div class="crm-activity-popup-timeline-checkbox-container">
							...
							//wcomm {
							CallFields::EchoCompletedForSliderEdit($activity);
							// } wcomm
                        ...
                        <div class="crm-activity-popup-info">
                            <? foreach ($provider::getFieldsForEdit($activity) as $field):
                                ...
                                switch ($field['TYPE'])
                                {
                                    ...
                                    <?
                                        break;
                                    case 'COMMUNICATIONS2': ?>
                                        <div class="crm-activity-popup-info-person-container">
                                            <span class="crm-activity-popup-info-person-text"><?=htmlspecialcharsbx($field['LABEL'])?>:</span>
                                            <div class="crm-activity-popup-info-person-block" data-role="communications2-container" data-communication-type="<?=$provider::getCommunicationType($activity['PROVIDER_TYPE_ID'])?>"></div><!--crm-activity-popup-info-person-block-->
                                        </div>
                                    <?
                                        break;
                            ...
                            <div class="crm-activity-popup-info-additional-container">
                                ...
                                <div class="crm-activity-popup-info-person-detail-container <?if ($arResult['ADDITIONAL_MODE']):?>crm-activity-person-detail-open<?endif;?>" data-role="additional-container">
                                    ...
                                    //rem1rem1 {
                                    CallFields::EchoFields($activity);
                                    // } rem1rem1
