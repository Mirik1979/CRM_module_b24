...
elseif($action === 'SAVE')
{
    ...
    \Bitrix\Main\Diag\Debug::writeToFile("save");
    \Bitrix\Main\Diag\Debug::writeToFile($clientData);
	...
	$enableProductRows = !(isset($_POST['SKIP_PRODUCT_DATA']) && strcasecmp($_POST['SKIP_PRODUCT_DATA'], 'Y') === 0);
	if($enableProductRows && !$isNew && !$isCopyMode && $conversionWizard === null)
	{
		if(\CCrmProductRow::AreEquals($productRows, $originalProductRows))
		{
...
elseif($action === 'GET_BINDING_INFOS')
{
    ...
    \Bitrix\Main\Diag\Debug::writeToFile("get binding");
    \Bitrix\Main\Diag\Debug::writeToFile($entityIDs);
	$data = array();
	foreach($entityIDs as $entityID)
	{
...
elseif($action === 'ADD_BINDING')
{
    ...
    \Bitrix\Main\Diag\Debug::writeToFile("add binding");
    \Bitrix\Main\Diag\Debug::writeToFile($entityIDs);
	foreach($entityIDs as $entityID)
	{
		$isReadPermitted = CCrmContact::CheckReadPermission($entityID, $userPermissions);
