...
switch ($action)
{
	...
	case 'ACTIVITY_SAVE':
        ...
        {
            $data = $result->getData();
            $entityId=(int)$_POST['data']['communications2'][0]['entityId'];
            $Title=trim($_POST['data']['communications2'][0]['entityTitle']);
            $Description=trim($_POST['data']['communications2'][0]['entityDescription']);
            if($data["ACTIVITY"]["ID"]>0 && $entityId>0){

                $fields=CommCompanyFactory::createFromArray([
                ...
                $list=$CommCompanyRepository->GetList([
                    "limit" => 1,
                    'filter'=>[
                        'UF_ACTIVITY_ID'=>$data["ACTIVITY"]["ID"],
                    ],
                ]);
                if(count($list)==1){
                    /**
                     ...
                     */
                    $el=$list[0];
                    $CommCompanyRepository->update($el->getId(),$fields);
                }elseif(count($list)==0){
                    $CommCompanyRepository->add($fields);
                }



            }elseif($data["ACTIVITY"]["ID"]>0){
                $list=$CommCompanyRepository->GetList([
                    "limit" => 1,
                    'filter'=>[
                        'UF_ACTIVITY_ID'=>$data["ACTIVITY"]["ID"],
                ...
                if(count($list)==1){
                     ...
                     */
                    $el=$list[0];
                    $CommCompanyRepository->Delete($el->getId());
                }
            }
