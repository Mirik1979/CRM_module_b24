...
if ($CrmPerms->HavePerm('CONSTRUCTION', BX_CRM_PERM_NONE, 'IMPORT'))
{
	ShowError(GetMessage('CRM_PERMISSION_DENIED'));
	return;
}

$arResult['TYPE_LIST'] = CCrmStatus::GetStatusList('CONSTRUCTION_TYPE');
...
if(!function_exists('__CrmImportPrepareDupControlTab'))
{
	function __CrmImportPrepareDupControlTab(&$arResult, $requisiteOptions)
	{
		...
		$arResult['FIELDS']['tab_3'][] = array(
			...
			'name' => GetMessage('CRM_FIELD_DUP_CONTROL_ENABLE_NAME'),
			'type' => 'checkbox',
			'value' => 'Y'
		);
...
if(!function_exists('__CrmImportConstructionAddressesToRequisite'))
{
	function __CrmImportConstructionAddressesToRequisite($constructionFields, $sourceFields, $dupCtrlType, $impAddrPresetId)
	{
		$result = new Result();

		$constructionAddressFields = array(
			'ADDRESS',
			'ADDRESS_2',
			'ADDRESS_CITY',
			'ADDRESS_POSTAL_CODE',
		...
		{
			$addrExists = false;
			foreach ($constructionAddressFields as $fieldName)
			{
			...
			if ($addrExists)
			{
				...
				foreach ($addressFields as $fieldName)
				{
					if ($fieldName === 'ADDRESS_1')
						$constructionFieldName = $addrFieldPref.'ADDRESS';
					else if ($fieldName === 'ADDRESS_2')
						$constructionFieldName = $addrFieldPref.$fieldName;
					else
						$constructionFieldName = $addrFieldPref.'ADDRESS_'.$fieldName;

					$address[$fieldName] =
						isset($sourceFields[$constructionFieldName]) ? $sourceFields[$constructionFieldName] : null;
				}
				$address['ANCHOR_TYPE_ID'] = CCrmOwnerType::Construction;
				$address['ANCHOR_ID'] = $constructionFields['ID'];
				...
				unset($address, $fieldName, $constructionFieldName);
			}
			unset($addrExists);
		}
		...
		if (!empty($addresses))
		{
			...
			try
			{
				$clientFullName = CCrmConstruction::PrepareFormattedName(
					array(
						'HONORIFIC' => isset($constructionFields['HONORIFIC']) ? $constructionFields['HONORIFIC'] : '',
						'NAME' => isset($constructionFields['NAME']) ? $constructionFields['NAME'] : '',
						'LAST_NAME' => isset($constructionFields['LAST_NAME']) ? $constructionFields['LAST_NAME'] : '',
						'SECOND_NAME' => isset($constructionFields['SECOND_NAME']) ? $constructionFields['SECOND_NAME'] : ''
					)
				);
				$rqImportResult = Requisite\ImportHelper::importOldRequisiteAddresses(
					CCrmOwnerType::Construction,
					$constructionFields['ID'],
					$dupCtrlType,
					$impAddrPresetId,
					array(
						EntityRequisite::COMPANY_NAME => isset($constructionFields['COMPANY_TITLE']) ? $constructionFields['COMPANY_TITLE'] : null,
						EntityRequisite::PERSON_FULL_NAME => $clientFullName,
						EntityRequisite::PERSON_FIRST_NAME => isset($constructionFields['NAME']) ? $constructionFields['NAME'] : null,
						EntityRequisite::PERSON_SECOND_NAME => isset($constructionFields['SECOND_NAME']) ? $constructionFields['SECOND_NAME'] : null,
						EntityRequisite::PERSON_LAST_NAME => isset($constructionFields['LAST_NAME']) ? $constructionFields['LAST_NAME'] : null,
...
/*$enableOutmodedFields = ConstructionSettings::getCurrent()->areOutmodedRequisitesEnabled();
...
else
{
	...
	$requisiteDefPresetId = EntityRequisite::getDefaultPresetId(CCrmOwnerType::Construction);
}
...
if(isset($_REQUEST['getSample']) && $_REQUEST['getSample'] == 'csv')
{
	...
	Header("Content-Disposition: attachment;filename=construction.csv");
	...
	$typeList = CCrmStatus::GetStatusListEx('CONSTRUCTION_TYPE');
...
elseif (isset($_REQUEST['import']) && isset($_SESSION['CRM_IMPORT_FILE']))
{
	...
	$arStatus['TYPE_LIST'] = CCrmStatus::GetStatusListEx('CONSTRUCTION_TYPE');
	...
	$usersByID = array();
	$usersByName = array();
	$defaultConstructionTypeID =  isset($_SESSION['CRM_IMPORT_DEFAULT_TYPE_ID']) ? $_SESSION['CRM_IMPORT_DEFAULT_TYPE_ID'] : '';
	...
	$dupChecker = new \Bitrix\Crm\Integrity\ConstructionDuplicateChecker();
	$processedQty = 0;

	$tempDir = isset($_SESSION['CRM_IMPORT_TEMP_DIR']) ? $_SESSION['CRM_IMPORT_TEMP_DIR'] : '';
	if($tempDir === '')
	{
		$tempDir = $_SESSION['CRM_IMPORT_TEMP_DIR'] = CTempFile::GetDirectoryName(1, array('crm', uniqid('construction_import_')));
	...
	while($arData = $csvFile->Fetch())
	{
		...
		{
			$arConstruction = $customFileImport->prepareConstruction($arData);
			if(isset($arConstruction['COMPANY_TITLE']) && $arConstruction['COMPANY_TITLE'] !== '')
			{
				$obRes = CCrmCompany::GetListEx(array(), array('TITLE' => $arConstruction['COMPANY_TITLE']), false, false, array('ID'));
				if (is_object($obRes) && ($arRow = $obRes->Fetch()) !== false)
				{
					$arConstruction['COMPANY_ID'] = $arRow['ID'];
				}
				else
				{
					//Try to create new company
					$companyEntity = new CCrmCompany(false);
					$companyFields = array('TITLE' => $arConstruction['COMPANY_TITLE']);
					...
					{
						$arConstruction['COMPANY_ID'] = $newCompanyID;
					}
				}
			}
		}
		else
		{
			...
			foreach ($arData as $key => $data)
			{
				if (isset($_SESSION['CRM_IMPORT_FILE_FIELD_'.$key]) && !empty($_SESSION['CRM_IMPORT_FILE_FIELD_'.$key]))
				{
					...
					{
						$arConstruction[$currentKey] = isset($arStatus['TYPE_LIST'][$data]) ? $data : array_search($data, $arStatus['TYPE_LIST']);
					}
					elseif ($currentKey == 'SOURCE_ID')
					{
						$arConstruction[$currentKey] = isset($arStatus['SOURCE_LIST'][$data]) ? $data : array_search($data, $arStatus['SOURCE_LIST']);
					}
					elseif ($currentKey == 'HONORIFIC')
					{
						$arConstruction[$currentKey] = isset($arStatus['HONORIFIC'][$data]) ? $data : array_search($data, $arStatus['HONORIFIC']);
					}
					elseif ($currentKey == 'PHOTO')
					{
						if(CCrmUrlUtil::HasScheme($data) && CCrmUrlUtil::IsSecureUrl($data))
						{
							...
							{
								$arConstruction[$currentKey] = array_merge($data, array('MODULE_ID' => 'crm'));
							}
						}
					}
					...
					{
						$arConstruction[$currentKey] = isset($arStatus[$currentKey.'_LIST'][$data])? $data: array_search($data, $arStatus[$currentKey.'_LIST']);
						if ($arConstruction[$currentKey] === false)
							unset($arConstruction[$currentKey]);
					}
					elseif ($currentKey  == 'FULL_NAME')
					{
						...
						{
							$data = explode(' ', $data);
							if (count($data) > 1 && (!isset($arConstruction['LAST_NAME']) || $arConstruction['LAST_NAME'] === ''))
							{
								$lastName = trim($data[1]);
								if ($lastName !== '')
								{
									$arConstruction['LAST_NAME'] = $lastName;
								}
							}

							if (!isset($arConstruction['NAME']) || $arConstruction['NAME'] === '')
							{
								$name = trim($data[0]);
								if($name !== '')
								{
									$arConstruction['NAME'] = $name;
								}
							}
						}
						unset($arConstruction[$currentKey]);
					...
					elseif ($currentKey == 'ASSIGNED_BY_ID')
					{
						...
						if($userID > 0)
						{
							$arConstruction['ASSIGNED_BY_ID'] = $userID;
						}
						elseif($defaultUserID > 0)
						{
							$arConstruction['ASSIGNED_BY_ID'] = $defaultUserID;
						}
					}
					else
					{
						// Finally try to internalize user type values
						$arConstruction[$currentKey] = $CCrmUserType->Internalize($currentKey, $data, ',');
					}
				}
			}
		}

		if (!isset($arConstruction['TYPE_ID']) && $defaultConstructionTypeID !== '')
		{
			$arConstruction['TYPE_ID'] = $defaultConstructionTypeID;
		}
		if (!isset($arConstruction['SOURCE_ID']) && $defaultSourceID !== '')
		{
			$arConstruction['SOURCE_ID'] = $defaultSourceID;
		}
		if (!isset($arConstruction['SOURCE_DESCRIPTION']) && $defaultSourceDescription !== '')
		{
			$arConstruction['SOURCE_DESCRIPTION'] = $defaultSourceDescription;
		}
		if (!isset($arConstruction['OPENED']) && $defaultOpened !== '')
		{
			$arConstruction['OPENED'] = $defaultOpened;
		}
		if (!isset($arConstruction['EXPORT']) && $defaultExport !== '')
		{
			$arConstruction['EXPORT'] = $defaultExport;
		}
		if (!isset($arConstruction['ASSIGNED_BY_ID']) && $defaultUserID > 0)
		{
			$arConstruction['ASSIGNED_BY_ID'] = $defaultUserID;
		}

		//Try to map full address to first address line
		if(isset($arConstruction['FULL_ADDRESS']) && !isset($arConstruction['ADDRESS']))
		{
			$arConstruction['ADDRESS'] = $arConstruction['FULL_ADDRESS'];
			unset($arConstruction['FULL_ADDRESS']);
		}

		CCrmFieldMulti::PrepareFields($arConstruction);
		$constructionSourceFields = $arConstruction;
		$isDuplicate = false;
...
//				$fieldNames[] = 'LAST_NAME';
				...
				&& is_object($requisiteImportHelper) && $requisiteImportHelper->isReady())
			{
				...
				{
					$arConstruction['RQ'] = $requisiteDupParams['DUP_PARAM_LIST'];
					$fieldNames = array_merge($fieldNames, $requisiteDupParams['DUP_PARAM_FIELDS']);
				}
				unset($requisiteDupParams);
			}*/

			$adapter = \Bitrix\Crm\EntityAdapterFactory::create($arConstruction, CCrmOwnerType::Construction);
			$dups = $dupChecker->findDuplicates($adapter, new \Bitrix\Crm\Integrity\DuplicateSearchParams($fieldNames));

			if ($importRequisite && $enableDupCtrlByRequisite
				&& is_object($requisiteImportHelper) && $requisiteImportHelper->isReady()
				&& $enableDupCtrlByRequisite && isset($arConstruction['RQ']))
			{
				unset($arConstruction['RQ']);
			}
			...
			foreach($dups as &$dup)
			{
				...
				if(empty($dupIDs))
				{
					$dupIDs = $dup->getEntityIDsByType(CCrmOwnerType::Construction);
				}
				else
				{
					$dupIDs = array_intersect($dupIDs, $dup->getEntityIDsByType(CCrmOwnerType::Construction));
				}
			}
			unset($dup);
			...
			if(!empty($dupIDs))
			{
				...
				{
					$dupItems = array();
					$dbResult = CCrmConstruction::GetListEx(array(), array('@ID' => $dupIDs, 'CHECK_PERMISSIONS' => 'Y'), false, false, array('*', 'UF_*'));
						...
						while($fields = $dbResult->Fetch())
						{
							if($loadMultiFields)
							{
								...
								$dbMultiFields = CCrmFieldMulti::GetList(
									array('ID' => 'asc'),
									array('ENTITY_ID' => CCrmOwnerType::ConstructionName, 'ELEMENT_ID' => $fields['ID'])
								);
					...
					$multiFields = isset($arConstruction['FM']) ? $arConstruction['FM'] : array();
					...
					foreach($dupItems as &$item)
					{
						foreach($mappedFields as $fieldName)
						{
							if($isRewrite)
							{
								...
								{
									if(isset($arConstruction['COMPANY_ID']))
									{
										$item['COMPANY_ID'] = $arConstruction['COMPANY_ID'];
									}
								}
								if($fieldName === 'PHOTO')
								{
									...
									$item['PHOTO'] = $arConstruction['PHOTO'];
								}
								elseif(isset($arConstruction[$fieldName]))
								{
									$item[$fieldName] = $arConstruction[$fieldName];
								}
							}
							else
							{
								...
								elseif(isset($arConstruction[$fieldName]) && !empty($arConstruction[$fieldName]))
								{
									if($fieldName === 'COMMENTS')
									{
										...
										if($comments === '')
										{
											$item['COMMENTS'] = $arConstruction['COMMENTS'];
										}
									}
						...
						{
							$importResult = __CrmImportConstructionAddressesToRequisite($item, $constructionSourceFields, $dupCtrlType, $impAddrPresetId);
						...
						if (!$errorOccured)
						{
							if(!$CCrmConstruction->Update($item['ID'], $item))
							{
								if ($importRequisite && is_object($requisiteImportHelper))
		...
		{
			$arConstruction['PERMISSION'] = 'IMPORT';

			$id = $CCrmConstruction->Add($arConstruction);
			...
			if(!$id)
			{
				...
				$arResult['error']++;
				$arResult['error_data'][] = Array(
					'message' => $arConstruction['RESULT_MESSAGE'],
					'data' => $errRows
				);
			...
			elseif(!empty($arConstruction))
			{
				if ($impAddrToRequisite)
				{
					$importResult = __CrmImportConstructionAddressesToRequisite($arConstruction, $constructionSourceFields, $dupCtrlType, $impAddrPresetId);
	...
	if($arResult['error'] > 0)
	{
		$arResult['errata_url'] = SITE_DIR.'local/components/citfact/crm.construction.import/show_file.php?name=errata';
	}

	if($enableDupFile && $arResult['duplicate'] > 0)
	{
		$arResult['duplicate_url'] = SITE_DIR.'local/components/citfact/crm.construction.import/show_file.php?name=duplicate';
	}
...
if($_SERVER['REQUEST_METHOD'] == 'POST' && check_bitrix_sessid())
{
	if(isset($_POST['next']))
	{
		if($arResult['STEP'] == 1)
		{
			...
			else
			{
				...
				else
				{
					...
					$defaultConstructionTypeID = isset($_POST['IMPORT_DEFAULT_TYPE_ID']) ? $_POST['IMPORT_DEFAULT_TYPE_ID'] : '';
					if($defaultConstructionTypeID !== '' && isset($arResult['TYPE_LIST'][$defaultConstructionTypeID]))
					{
						$_SESSION['CRM_IMPORT_DEFAULT_TYPE_ID'] = $defaultConstructionTypeID;
					}
		...
		else if ($arResult['STEP'] == 3)
		{
			...
			ob_start();
			?>
			<div class="crm_import_entity"><?=GetMessage('CRM_IMPORT_FINISH')?>: <span id="crm_import_entity">0</span> <span id="crm_import_entity_progress"><img src="/local/components/citfact/crm.construction.import/templates/.default/images/wait.gif" align="absmiddle"></span></div>
		...
		else if ($arResult['STEP'] == 4)
		{
			...
			LocalRedirect(CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_CONSTRUCTION_LIST'], array()));
		}
		else
		{
			$arResult['STEP'] = 1;
...
$arResult['FORM_ID'] = 'CRM_CONSTRUCTION_IMPORT';
...
/*$arResult['FIELDS']['tab_1'][] = array(
	...
	'componentParams' => array(
		'NAME' => 'crm_construction_import_responsible',
...
if(!$enableFixedOrigin)
{
	...
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'value' => '<a href="?getSample=csv&ncc=1" '.
			'onclick="'.htmlspecialcharsbx('BX.Crm.ConstructionImportSampleLink.items["'.
				CUtil::JSEscape($arResult['IMPORT_SAMPLE_LINK_ID']).'"].getSample("'.
				CUtil::JSEscape(
					CHTTP::urlAddParams(
						CComponentEngine::makePathFromTemplate($arParams['PATH_TO_CONSTRUCTION_IMPORT']),
...
/*if ($enableOutmodedFields)
{
	...
	if (isset($_POST['IMPORT_ADDR_PRESET']))
		...
		$defAddrPresetId = EntityRequisite::getDefaultPresetId(CCrmOwnerType::Construction);
