...
class CCrmConstructionDetailsComponent extends CBitrixComponent
{
	/** @var string */
	protected $guid = '';
	/** @var int */
	...
	public function __construct($component = null)
	{
		...
		$this->userFieldEntityID = \CCrmConstruction::GetUserFieldEntityID();
	...
	public function executeComponent()
	{
		...
		$this->enableOutmodedFields = \Bitrix\Crm\Settings\ConstructionSettings::getCurrent()->areOutmodedRequisitesEnabled();
		...
		$this->arResult['PATH_TO_CONSTRUCTION_SHOW'] = \CrmCheckPath(
			'PATH_TO_CONSTRUCTION_SHOW',
			$this->arParams['PATH_TO_CONSTRUCTION_SHOW'],
			$APPLICATION->GetCurPage().'?construction_id=#construction_id#&show'
		);
		$this->arResult['PATH_TO_CONSTRUCTION_EDIT'] = \CrmCheckPath(
			'PATH_TO_CONSTRUCTION_EDIT',
			$this->arParams['PATH_TO_CONSTRUCTION_EDIT'],
			$APPLICATION->GetCurPage().'?construction_id=#construction_id#&edit'
		);
		...
		$this->arResult['CONTEXT_ID'] = \CCrmOwnerType::ConstructionName.'_'.$this->arResult['ENTITY_ID'];
		...
		if($this->entityID > 0)
		{
			if(!\CCrmConstruction::Exists($this->entityID))
			{
				ShowError(GetMessage('CRM_CONSTRUCTION_NOT_FOUND'));
				return;
			}
			...
			if($this->request->get('copy') !== null)
			{
				...
				$this->arResult['CONTEXT_PARAMS']['CONSTRUCTION_ID'] = $this->entityID;
			}
			else
			{
		...
		$this->enableDupControl = $this->arResult['DUPLICATE_CONTROL']['enabled'] =
			!$this->isEditMode && \Bitrix\Crm\Integrity\DuplicateControl::isControlEnabledFor(CCrmOwnerType::Construction);

		if($this->enableDupControl)
		{
			$this->arResult['DUPLICATE_CONTROL']['serviceUrl'] = '/local/components/citfact/crm.construction.edit/ajax.php?'.bitrix_sessid_get();
			$this->arResult['DUPLICATE_CONTROL']['entityTypeName'] = CCrmOwnerType::ConstructionName;
			$this->arResult['DUPLICATE_CONTROL']['groups'] = array(
				'fullName' => array(
					'groupType' => 'fullName',
					'groupSummaryTitle' => Loc::getMessage('CRM_CONSTRUCTION_DUP_CTRL_FULL_NAME_SUMMARY_TITLE')
				),
				'email' => array(
					'groupType' => 'communication',
					'communicationType' => 'EMAIL',
					'groupSummaryTitle' => Loc::getMessage('CRM_CONSTRUCTION_DUP_CTRL_EMAIL_SUMMARY_TITLE')
				),
				'phone' => array(
					'groupType' => 'communication',
					'communicationType' => 'PHONE',
					'groupSummaryTitle' => Loc::getMessage('CRM_CONSTRUCTION_DUP_CTRL_PHONE_SUMMARY_TITLE')
				)
			);
		}
		//endregion
		...
		if($this->leadID > 0)
		{
			...
			if($this->conversionWizard !== null)
			{
				$this->arResult['CONTEXT_PARAMS'] = array_merge(
					...
					$this->conversionWizard->prepareEditorContextParams(\CCrmOwnerType::Construction)
				);
			}
		}
		//endregion
		...
		if($this->isCopyMode)
		{
			if(!(\CCrmConstruction::CheckReadPermission($this->entityID, $this->userPermissions)
				&& \CCrmConstruction::CheckCreatePermission($this->userPermissions))
			)
			{
		...
		{
			if(\CCrmConstruction::CheckUpdatePermission($this->entityID, $this->userPermissions))
			{
				$this->arResult['READ_ONLY'] = false;
			}
			elseif(\CCrmConstruction::CheckReadPermission($this->entityID, $this->userPermissions))
			{
		...
		{
			if(\CCrmConstruction::CheckCreatePermission($this->userPermissions))
			{
				$this->arResult['READ_ONLY'] = false;
			}
			else
		...
		$this->guid = $this->arResult['GUID'] = isset($this->arParams['GUID'])
			? $this->arParams['GUID'] : "construction_{$this->entityID}_details";

		$this->arResult['EDITOR_CONFIG_ID'] = isset($this->arParams['EDITOR_CONFIG_ID'])
			? $this->arParams['EDITOR_CONFIG_ID'] : 'construction_details';
		//endregion
		...
		$this->arResult['ENTITY_INFO'] = array(
			...
			'SHOW_URL' => CCrmOwnerType::GetEntityShowPath(CCrmOwnerType::Construction, $this->entityID, false),
		);
		//endregion
		...
		if($this->isCopyMode)
		{
			$APPLICATION->SetTitle(Loc::getMessage('CRM_CONSTRUCTION_COPY_PAGE_TITLE'));
		}
		...
		elseif(!$this->isEditMode)
		{
			$APPLICATION->SetTitle(Loc::getMessage('CRM_CONSTRUCTION_CREATION_PAGE_TITLE'));
		}
		//endregion
		...
		$this->arResult['ENTITY_CONFIG'] = array(
			array(
				'name' => 'main',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_SECTION_MAIN'),
				'type' => 'section',
				'elements' =>
					array_merge(
						array(
...
//							array('name' => 'REQUISITES'),
			...
			/*array(
				'name' => 'additional',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_SECTION_ADDITIONAL'),
				'type' => 'section',
				'elements' =>
					array_merge(
						array(
		...
		if($this->entityID > 0)
		{
			$this->arResult['TABS'][] = array(
				'id' => 'tab_deal',
				'name' => Loc::getMessage('CRM_CONSTRUCTION_TAB_DEAL'),
				'loader' => array(
					...
					'componentData' => array(
						...
						'params' => array(
							...
							'INTERNAL_FILTER' => array('CONSTRUCTION_ID' => $this->entityID),
							'INTERNAL_CONTEXT' => array('CONSTRUCTION_ID' => $this->entityID),
							'GRID_ID_SUFFIX' => 'CONSTRUCTION_DETAILS',
							'TAB_ID' => 'tab_deal',
							'NAME_TEMPLATE' => $this->arResult['NAME_TEMPLATE'],
							'ENABLE_TOOLBAR' => true,
							'PRESERVE_HISTORY' => true,
							'ADD_EVENT_NAME' => 'CrmCreateDealFromConstruction'
						)
					)
				)
			);
			$this->arResult['TABS'][] = array(
				'id' => 'tab_quote',
				'name' => Loc::getMessage('CRM_CONSTRUCTION_TAB_QUOTE'),
				'loader' => array(
					...
					'componentData' => array(
						...
						'params' => array(
							...
							'ENABLE_TOOLBAR' => true,
							'PRESERVE_HISTORY' => true,
							'ADD_EVENT_NAME' => 'CrmCreateQuoteFromConstruction'
						)
					)
				)
			);
			$this->arResult['TABS'][] = array(
				'id' => 'tab_invoice',
				'name' => Loc::getMessage('CRM_CONSTRUCTION_TAB_INVOICES'),
				'loader' => array(
					...
					'componentData' => array(
						...
						'params' => array(
							...
							'PATH_TO_CONSTRUCTION_EDIT' => $this->arResult['PATH_TO_CONSTRUCTION_EDIT'],
							'PATH_TO_DEAL_EDIT' => $this->arResult['PATH_TO_DEAL_EDIT'],
							'PATH_TO_INVOICE_EDIT' => $this->arResult['PATH_TO_INVOICE_EDIT'],
							'PATH_TO_INVOICE_PAYMENT' => $this->arResult['PATH_TO_INVOICE_PAYMENT'],
							'INTERNAL_FILTER' => array('UF_CONSTRUCTION_ID' => $this->entityID),
							...
							'ENABLE_TOOLBAR' => 'Y',
							'PRESERVE_HISTORY' => true,
							'ADD_EVENT_NAME' => 'CrmCreateInvoiceFromConstruction'
						)
					)
				)
			);
			if (CModule::IncludeModule('sale') && Main\Config\Option::get("crm", "crm_shop_enabled") === "Y")
			{
				$this->arResult['TABS'][] = array(
					'id' => 'tab_order',
					'name' => Loc::getMessage('CRM_CONSTRUCTION_TAB_ORDERS'),
					'loader' => array(
						...
						'componentData' => array(
							...
							'params' => array(
								...
								'PATH_TO_INVOICE_EDIT' => $this->arResult['PATH_TO_INVOICE_EDIT'],
								'PATH_TO_INVOICE_PAYMENT' => $this->arResult['PATH_TO_INVOICE_PAYMENT'],
								'INTERNAL_FILTER' => array('ASSOCIATED_CONSTRUCTION_ID' => $this->entityID),
								'SUM_PAID_CURRENCY' => \CCrmCurrency::GetBaseCurrencyID(),
								'GRID_ID_SUFFIX' => 'CONSTRUCTION_DETAILS',
								'TAB_ID' => 'tab_order',
								'NAME_TEMPLATE' => $this->arResult['NAME_TEMPLATE'],
								'ENABLE_TOOLBAR' => 'Y',
								'PRESERVE_HISTORY' => true,
								'ADD_EVENT_NAME' => 'CrmCreateOrderFromConstruction'
							)
						)
					)
				);
			...
			if (CModule::IncludeModule('bizproc') && CBPRuntime::isFeatureEnabled())
			{
				$this->arResult['TABS'][] = array(
					'id' => 'tab_bizproc',
					'name' => Loc::getMessage('CRM_CONSTRUCTION_TAB_BIZPROC'),
					'loader' => array(
						...
						'componentData' => array(
							...
							'params' => array(
								...
								'DOCUMENT_ID' => 'CONSTRUCTION_'.$this->entityID
							)
						)
					)
				);
				$this->arResult['BIZPROC_STARTER_DATA'] = array(
					'templates' => CBPDocument::getTemplatesForStart(
						...
						array('crm', 'CCrmDocumentConstruction', 'CONSTRUCTION_'.$this->entityID)
					),
					'moduleId' => 'crm',
					'entity' => 'CCrmDocumentConstruction',
					'documentType' => 'CONSTRUCTION',
					'documentId' => 'CONSTRUCTION_'.$this->entityID
				);
			}
			/*$this->arResult['TABS'][] = array(
				'id' => 'tab_tree',
				'name' => Loc::getMessage('CRM_CONSTRUCTION_TAB_TREE'),
				'loader' => array(
			...
			$this->arResult['TABS'][] = array(
				'id' => 'tab_event',
				'name' => Loc::getMessage('CRM_CONSTRUCTION_TAB_EVENT'),
				'loader' => array(
					'serviceUrl' => '/bitrix/components/bitrix/crm.event.view/lazyload.ajax.php?&site'.SITE_ID.'&'.bitrix_sessid_get(),
					'componentData' => array(
						'template' => '',
						'contextId' => "CONSTRUCTION_{$this->entityID}_EVENT",
						'params' => array(
							'AJAX_OPTION_ADDITIONAL' => "CONSTRUCTION_{$this->entityID}_EVENT",
							'ENTITY_TYPE' => CCrmOwnerType::ConstructionName,
							'ENTITY_ID' => $this->entityID,
							'PATH_TO_USER_PROFILE' => $this->arResult['PATH_TO_USER_PROFILE'],
							'TAB_ID' => 'tab_event',
							'INTERNAL' => 'Y',
			...
			/*$this->arResult['TABS'][] = array(
				'id' => 'tab_portrait',
				'name' => Loc::getMessage('CRM_CONSTRUCTION_TAB_PORTRAIT'),
				'loader' => array(
					...
					'componentData' => array(
						...
						'params' => array(
							...
							'ELEMENT_TYPE' => CCrmOwnerType::Construction,
							'IS_FRAME' => 'Y'
						)
					)
				)
			...
			{
				$listIblock = CLists::getIblockAttachedCrm(CCrmOwnerType::ConstructionName);
					...
					$this->arResult['TABS'][] = array(
						...
						'loader' => array(
							...
							'componentData' => array(
								...
								'params' => array(
									'ENTITY_ID' => $this->entityID,
									'ENTITY_TYPE' => CCrmOwnerType::Construction,
		...
		{
			CCrmEvent::RegisterViewEvent(CCrmOwnerType::Construction, $this->entityID, $this->userID);
		}
		//endregion
	...
	public function prepareEntityDataScheme()
	{
		...
		{
			$this->entityDataScheme = \CCrmConstruction::GetFieldsInfo();
	...
	public function prepareFieldInfos()
	{
		...
		$this->arResult['ENTITY_FIELDS'] = array(
			array(
				'name' => 'ID',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_ID'),
				'type' => 'text',
				'editable' => false,
			...
			/*array(
				'name' => 'HONORIFIC',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_HONORIFIC'),
				'type' => 'list',
				'editable' => true,
				...
				'data' => array(
					'items'=> \CCrmInstantEditorHelper::PrepareListOptions(
						...
						array('NOT_SELECTED' => Loc::getMessage('CRM_CONSTRUCTION_HONORIFIC_NOT_SELECTED'))
					)
				)
			),*/
			/*array(
				'name' => 'LAST_NAME',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_LAST_NAME'),
				'type' => 'text',
				'isHeading' => true,
				'visibilityPolicy' => 'edit',
			...
			array(
				'name' => 'NAME',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_NAME'),
				'type' => 'text',
				'visibilityPolicy' => 'edit',
				'requiredConditionally' => true,
				'editable' => true,
			...
			/*array(
				'name' => 'SECOND_NAME',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_SECOND_NAME'),
				'type' => 'text',
				'visibilityPolicy' => 'edit',
				'editable' => true,
			...
			/*array(
				'name' => 'PHOTO',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_PHOTO'),
				'type' => 'image',
				'editable' => true,
			...
			/*array(
				'name' => 'BIRTHDATE',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_BIRTHDATE'),
				'type' => 'datetime',
				'editable' => true,
			...
			array(
				'name' => 'POST',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_POST'),
				'type' => 'text',
				'editable' => true
			),
			array(
				'name' => 'COMMENTS',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_COMMENTS'),
				'type' => 'html',
				'editable' => true
			),*/
			array(
				'name' => 'ASSIGNED_BY_ID',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_ASSIGNED_BY_ID'),
				'type' => 'user',
				'editable' => true,
				'data' => array(
			...
			/*array(
				'name' => 'OPENED',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_OPENED'),
				'type' => 'boolean',
				'editable' => true
			),
			array(
				'name' => 'EXPORT',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_EXPORT'),
				'type' => 'boolean',
				'editable' => true
			),
			array(
				'name' => 'TYPE_ID',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_TYPE_ID'),
				'type' => 'list',
				'editable' => true,
				'data' => array('items'=> \CCrmInstantEditorHelper::PrepareListOptions(CCrmStatus::GetStatusList('CONSTRUCTION_TYPE')))
			),
			array(
				'name' => 'SOURCE_ID',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_SOURCE_ID'),
				'type' => 'list',
				'editable' => true,
			...
			array(
				...
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_SOURCE_DESCRIPTION'),
				'type' => 'text',
			...
			/*array(
				'name' => 'COMPANY',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_COMPANY'),
				'type' => 'client',
				'editable' => true,
				'data' => array(
					'map' => array(
					...
					'secondaryEntityLegend' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_COMPANY_LEGEND'),
					'loaders' => array(
						'primary' => array(
			...
			/*array(
				'name' => 'REQUISITES',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_REQUISITES'),
				'type' => 'requisite_list',
				'editable' => true,
				'data' => array(
		...
		$this->arResult['ENTITY_FIELDS'][] = array(
			'name' => 'UTM',
			'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_UTM'),
			'type' => 'custom',
		...
		/*if($this->enableOutmodedFields)
		{
			$this->arResult['ENTITY_FIELDS'][] = array(
				'name' => 'ADDRESS',
				'title' => Loc::getMessage('CRM_CONSTRUCTION_FIELD_ADDRESS'),
				'type' => 'address',
				'editable' => true,
				'data' => array(
					'fields' => array(
	...
	public function prepareEntityData()
	{
		...
		{
			$this->entityData = array();
			$this->entityData['OPENED'] = \Bitrix\Crm\Settings\ConstructionSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N';
		...
		else
		{
			...
			$this->entityData['FORMATTED_NAME'] =
				\CCrmConstruction::PrepareFormattedName(
					array(
		...
		if($this->entityID > 0)
		{
			...
			$entityKey = CCrmOwnerType::Construction.'_'.$this->entityID;
		...
		if($this->entityID > 0)
		{
			$companyIDs = Bitrix\Crm\Binding\ConstructionCompanyTable::getConstructionCompanyIDs($this->entityID);
		}
		...
		$this->entityData['ADDRESS_HTML'] = ConstructionAddressFormatter::format(
			$this->entityData,
			array(
