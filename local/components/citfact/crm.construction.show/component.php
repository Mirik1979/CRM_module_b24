...
if ($arResult['ELEMENT_ID'] <= 0 || !CCrmConstruction::CheckReadPermission($arResult['ELEMENT_ID'], $userPermissions))
{
...
$arResult['CAN_EDIT'] = CCrmConstruction::CheckUpdatePermission($arResult['ELEMENT_ID'], $userPermissions);
$arResult['TACTILE_FORM_ID'] = 'CRM_CONSTRUCTION_EDIT_V12';
...
$arResult['PATH_TO_CONSTRUCTION_SHOW'] = $arParams['PATH_TO_CONSTRUCTION_SHOW'] = CrmCheckPath('PATH_TO_CONSTRUCTION_SHOW', $arParams['PATH_TO_CONSTRUCTION_SHOW'], $APPLICATION->GetCurPage().'?construction_id=#construction_id#&show');
$arParams['PATH_TO_CONSTRUCTION_EDIT'] = CrmCheckPath('PATH_TO_CONSTRUCTION_EDIT', $arParams['PATH_TO_CONSTRUCTION_EDIT'], $APPLICATION->GetCurPage().'?construction_id=#construction_id#&edit');
...
$obFields = CCrmConstruction::GetListEx(
	array(),
...
$dbResMultiFields = CCrmFieldMulti::GetList(
	array('ID' => 'asc'),
	array('ENTITY_ID' => 'CONSTRUCTION', 'ELEMENT_ID' => $arResult['ELEMENT_ID'])
);
...
$arFields['~FORMATTED_NAME'] = CCrmConstruction::PrepareFormattedName(
	array(
...
$arFields['COMPANY_IDS'] = \Bitrix\Crm\Binding\ConstructionCompanyTable::getConstructionCompanyIDs($arResult['ELEMENT_ID']);
...
$arResult['FORM_ID'] = 'CRM_CONSTRUCTION_SHOW_V12';
$arResult['GRID_ID'] = 'CRM_CONSTRUCTION_LIST_V12';
$arResult['BACK_URL'] = $arParams['PATH_TO_CONSTRUCTION_LIST'];
...
$arResult['FIELDS']['tab_1'][] = array(
	'id' => 'section_construction_info',
	'name' => GetMessage('CRM_SECTION_CONSTRUCTION_INFO_2'),
	'type' => 'section',
	'isTactile' => true
);
...
$arResult['FIELDS']['tab_1'][] = array(
	'id' => 'BIRTHDATE',
	'name' => GetMessage('CRM_CONSTRUCTION_SHOW_FIELD_BIRTHDATE'),
	'type' => 'label',
...
if (CCrmCompany::CheckReadPermission(0, $userPermissions))
{
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'componentParams' => array(
			'OWNER_TYPE_NAME' => CCrmOwnerType::ConstructionName,
			'OWNER_ID' => $arParams['ELEMENT_ID'],
			'CONTEXT' => $arParams['ELEMENT_ID'] > 0 ? "CONSTRUCTION_{$arParams['ELEMENT_ID']}" : 'NEWCONSTRUCTION',
			'READ_ONLY'=> true,
...
if($enableOutmodedFields)
{
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'type' => 'custom',
		'value' =>  ConstructionAddressFormatter::format(
			$arResult['ELEMENT'],
...
{
	$arResult['LIST_IBLOCK'] = CLists::getIblockAttachedCrm('CONSTRUCTION');
	foreach($arResult['LIST_IBLOCK'] as $iblockId => $iblockName)
	{
		$arResult['LISTS'] = true;
		$arResult['FIELDS']['tab_lists_'.$iblockId][] = array(
			'id' => 'CONSTRUCTION_LISTS_'.$iblockId,
			'name' => $iblockName,
			'colspan' => true,
	...
	if ($bizprocTask !== '')
	{
		...
		$APPLICATION->IncludeComponent(
			...
			Array(
				...
				'WORKFLOW_ID' => '',
				'DOCUMENT_URL' =>  CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_CONSTRUCTION_SHOW'],
					array('construction_id' => $arResult['ELEMENT']['ID'])
				),
				'SET_TITLE' => 'Y',
				'SET_NAV_CHAIN' => 'Y',
				'NAME_TEMPLATE' => $arParams['NAME_TEMPLATE']
		...
		$arResult['FIELDS']['tab_bizproc'][] = array(
			'id' => 'CONSTRUCTION_BIZPROC',
			'name' => GetMessage('CRM_FIELD_CONSTRUCTION_BIZPROC'),
			'colspan' => true,
			'type' => 'custom',
			'value' => $sVal
		);
	...
	elseif (isset($_REQUEST['bizproc_log']) && strlen($_REQUEST['bizproc_log']) > 0)
	{
		...
		$APPLICATION->IncludeComponent('bitrix:bizproc.log',
			...
			Array(
				...
				'COMPONENT_VERSION' => 2,
				'DOCUMENT_ID' => 'CONSTRUCTION_'.$arResult['ELEMENT']['ID'],
		...
		else
		{
			...
			$APPLICATION->IncludeComponent('bitrix:bizproc.document',
				...
				Array(
					...
					'DOCUMENT_TYPE' => 'CONSTRUCTION',
					'DOCUMENT_ID' => 'CONSTRUCTION_'.$arResult['ELEMENT']['ID'],
					'TASK_EDIT_URL' => CHTTP::urlAddParams(CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_CONSTRUCTION_SHOW'],
						array(
							'construction_id' => $arResult['ELEMENT']['ID']
						)),
						array('bizproc_task' => '#ID#', $formTabKey => 'tab_bizproc')
					),
					'WORKFLOW_LOG_URL' => CHTTP::urlAddParams(CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_CONSTRUCTION_SHOW'],
						array(
							'construction_id' => $arResult['ELEMENT']['ID']
						)),
						array('bizproc_log' => '#ID#', $formTabKey => 'tab_bizproc')
					),
					'WORKFLOW_START_URL' => CHTTP::urlAddParams(CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_CONSTRUCTION_SHOW'],
						array(
							'construction_id' => $arResult['ELEMENT']['ID']
						)),
						array('bizproc_start' => 1, $formTabKey => 'tab_bizproc')
					),
					'back_url' => CHTTP::urlAddParams(CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_CONSTRUCTION_SHOW'],
						array(
							'construction_id' => $arResult['ELEMENT']['ID']
						)),
...
if ($companyID > 0 && CCrmCompany::CheckReadPermission(0, $userPermissions))
{
	$arResult['FIELDS']['tab_company'][] = array(
		...
		'name' => GetMessage('CRM_FIELD_CONSTRUCTION_COMPANY'),
		'colspan' => true,
		...
		'componentData' => array(
			...
			'params' => array(
				...
				'INTERNAL_FILTER' => array('ASSOCIATED_CONSTRUCTION_ID' => $arResult['ELEMENT']['ID']),
				'INTERNAL_ID' => $arResult['ELEMENT']['COMPANY_ID'],
				'INTERNAL_CONTEXT' => array('CONSTRUCTION_ID' => $arResult['ELEMENT']['ID']),
				'GRID_ID_SUFFIX' => 'CONSTRUCTION_SHOW',
				'FORM_ID' => $arResult['FORM_ID'],
				'TAB_ID' => 'tab_company',
...
if (CCrmDeal::CheckReadPermission(0, $userPermissions))
{
	$arResult['FIELDS']['tab_deal'][] = array(
		'id' => 'CONSTRUCTION_DEAL',
		'name' => GetMessage('CRM_FIELD_CONSTRUCTION_DEAL'),
		'colspan' => true,
		'type' => 'crm_deal_list',
		'componentData' => array(
			'template' => '',
...
if (CCrmQuote::CheckReadPermission(0, $userPermissions))
{
	$arResult['FIELDS']['tab_quote'][] = array(
		...
		'name' => GetMessage('CRM_FIELD_CONSTRUCTION_QUOTE'),
		'colspan' => true,
		'type' => 'crm_quote_list',
		'componentData' => array(
			'template' => '',
...
if (CCrmInvoice::CheckReadPermission(0, $userPermissions))
{
	$arResult['FIELDS']['tab_invoice'][] = array(
		...
		'name' => GetMessage('CRM_FIELD_CONSTRUCTION_INVOICE'),
		'colspan' => true,
		...
		'componentData' => array(
			...
			'params' => array(
				...
				'INTERNAL_FILTER' => array('UF_CONSTRUCTION_ID' => $arResult['ELEMENT']['ID']),
				'GRID_ID_SUFFIX' => 'CONSTRUCTION_SHOW',
				'FORM_ID' => $arResult['FORM_ID'],
				'TAB_ID' => 'tab_invoice',
				'NAME_TEMPLATE' => $arParams['NAME_TEMPLATE'],
				'ENABLE_TOOLBAR' => 'Y',
				'INTERNAL_ADD_BTN_TITLE' => GetMessage('CRM_CONSTRUCTION_ADD_INVOICE_TITLE'),
				'PRESERVE_HISTORY' => true
			)
		)
	);
...
$arResult['FIELDS']['tab_event'][] = array(
	'id' => 'section_event_construction',
	'name' => GetMessage('CRM_SECTION_EVENT'),
	'type' => 'section'
);

$arResult['FIELDS']['tab_event'][] = array(
	'id' => 'CONSTRUCTION_EVENT',
	'name' => GetMessage('CRM_FIELD_CONSTRUCTION_EVENT'),
	'colspan' => true,
	'type' => 'crm_event_view',
	'componentData' => array(
		'template' => '',
		'enableLazyLoad' => true,
		'contextId' => "CONSTRUCTION_{$arResult['ELEMENT']['ID']}_EVENT",
		'params' => array(
			'AJAX_OPTION_ADDITIONAL' => "CONSTRUCTION_{$arResult['ELEMENT']['ID']}_EVENT",
...
$activityBindings = array(array('TYPE_NAME' => CCrmOwnerType::ConstructionName, 'ID' => $arParams['ELEMENT_ID']));
foreach(CCrmLead::GetAssociatedIDs(CCrmOwnerType::Construction, $arParams['ELEMENT_ID']) as $leadID)
{
	$activityBindings[] = array('TYPE_NAME' => CCrmOwnerType::LeadName, 'ID' => $leadID);
}
$arResult['FIELDS']['tab_activity'][] = array(
	'id' => 'CONSTRUCTION_ACTIVITY_GRID',
	...
	'componentData' => array(
		...
		'params' => array(
			...
			'PREFIX' => 'CONSTRUCTION_ACTIONS_GRID',
...
$arResult['FIELDS']['tab_requisite'][] = array(
	'id' => 'CONSTRUCTION_REQUISITE',
	'name' => GetMessage('CRM_FIELD_CONSTRUCTION_REQUISITE'),
	'colspan' => true,
	...
	'componentData' => array(
		...
		'params' => array(
			...
			'INTERNAL_FILTER' => array('=ENTITY_TYPE_ID' => CCrmOwnerType::Construction, '=ENTITY_ID' => $arResult['ELEMENT']['ID']),
			'INTERNAL_CONTEXT' => array('CONSTRUCTION_ID' => $arResult['ELEMENT']['ID']),
			'GRID_ID_SUFFIX' => 'CONSTRUCTION_SHOW',
			'FORM_ID' => $arResult['FORM_ID'],
...
{
	CCrmEvent::RegisterViewEvent(CCrmOwnerType::Construction, $arParams['ELEMENT_ID'], $currentUserID);
}
