...
$APPLICATION->IncludeComponent(
	...
	array(
		...
		'SERVICE_URL' => '/local/components/citfact/crm.construction.show/ajax.php?'.bitrix_sessid_get(),
		'SHOW_SETTINGS' => 'Y'
	),
	$component,
	array('HIDE_ICONS' => 'Y')
			...
			function()
			{
				...
				var instantEditor = BX.CrmInstantEditor.create(
						'<?=CUtil::JSEscape($instantEditorID)?>',
						{
							...
							url: '/local/components/citfact/crm.construction.show/ajax.php?<?=bitrix_sessid_get()?>',
				...
				if(fullNameField)
				{
					var fullNameEditor = BX.CrmConstructionEditor.create(
							'fullNameEditor',
							{
								'serviceUrl': '<?='/local/components/citfact/crm.construction.edit/ajax.php?siteID='.SITE_ID.'&'.bitrix_sessid_get()?>',
								'actionName': 'SAVE_CONSTRUCTION',
								...
								'dialog': <?=CUtil::PhpToJSObject(
									array(
										'addButtonName' => GetMessageJS('CRM_CONSTRUCTION_EDIT_DLG_BTN_SAVE'),
										'cancelButtonName' => GetMessageJS('CRM_CONSTRUCTION_EDIT_DLG_BTN_CANCEL'),
										'title' => GetMessageJS('CRM_CONSTRUCTION_EDIT_DLG_TITLE'),
										'lastNameTitle' => GetMessageJS('CRM_CONSTRUCTION_EDIT_DLG_FIELD_LAST_NAME'),
										'nameTitle' => GetMessageJS('CRM_CONSTRUCTION_EDIT_DLG_FIELD_NAME'),
										'secondNameTitle' => GetMessageJS('CRM_CONSTRUCTION_EDIT_DLG_FIELD_SECOND_NAME'),
										'enableEmail' => false,
										'enablePhone' => false,
										'enableExport' => false
									)
				...
				if(BX(bpContainerId))
				{
					BX.CrmBizprocDispatcher.create(
						"<?=CUtil::JSEscape($bizprocDispatcherID)?>",
						{
							...
							serviceUrl: "/local/components/citfact/crm.construction.show/bizproc.php?construction_id=<?=$arResult['ELEMENT_ID']?>&post_form_uri=<?=urlencode($arResult['POST_FORM_URI'])?>&<?=bitrix_sessid_get()?>",
							formID: "<?=CUtil::JSEscape($arResult['FORM_ID'])?>",
							pathToEntityShow: "<?=CUtil::JSEscape($arResult['PATH_TO_CONSTRUCTION_SHOW'])?>"
						}
					);
				}
				<?endif;?>
	...
	BX.ready(function(){
		...
		BX.CrmEntityTreeDispatcher.create(
			'dispatcher<?= CUtil::JSEscape($treeDispatcherID)?>',
			{
				...
				pathToConstructionShow: '<?= CUtil::JSEscape($arParams['PATH_TO_CONSTRUCTION_SHOW'])?>',
