...
if(!function_exists('__CrmConstructionEditEndResponse'))
{
	function __CrmConstructionEditEndResponse($result)
	{
...
{
	__CrmConstructionEditEndResponse(array('ERROR' => 'Could not include crm module.'));
}
/*
...
{
	__CrmConstructionEditEndResponse(array('ERROR' => 'Access denied.'));
}
if ($_SERVER['REQUEST_METHOD'] != 'POST')
{
	__CrmConstructionEditEndResponse(array('ERROR' => 'Request method is not allowed.'));
}
...
if($action === 'SAVE_CONSTRUCTION')
{
	...
	$CrmConstruction = new CCrmConstruction();
	if($ID > 0)
	{
		if($CrmConstruction->Update($ID, $arFields, true, array('DISABLE_USER_FIELD_CHECK' => true, 'REGISTER_SONET_EVENT' => true)))
		{
			$info = CCrmEntitySelectorHelper::PrepareEntityInfo('CONSTRUCTION', $ID);
	...
	{
		$ID = $CrmConstruction->Add($arFields, true, array('DISABLE_USER_FIELD_CHECK' => true, 'REGISTER_SONET_EVENT' => true));
		if(is_int($ID) && $ID > 0)
		{
			$data['id'] = $ID;
		...
		{
			echo CUtil::PhpToJSObject(
				array('ERROR' => $CrmConstruction->LAST_ERROR)
			);
		}
	}
}
elseif($action === 'ENABLE_SONET_SUBSCRIPTION')
{
	...
	if($userID > 0 && $entityTypeName === CCrmOwnerType::ConstructionName && $entityID > 0 && CCrmConstruction::CheckReadPermission($entityID))
	{
		...
		if($isEnabled !== $enable)
		{
			if($enable)
			{
				CCrmSonetSubscription::RegisterSubscription(CCrmOwnerType::Construction, $entityID, CCrmSonetSubscriptionType::Observation, $userID);
			}
			else
			{
				CCrmSonetSubscription::UnRegisterSubscription(CCrmOwnerType::Construction, $entityID, CCrmSonetSubscriptionType::Observation, $userID);
			}
		}
	}
}
elseif($action === 'FIND_DUPLICATES')
{
	...
	if($entityTypeName === '')
	{
		__CrmConstructionEditEndResponse(array('ERROR' => 'Entity type is not specified.'));
	}

	$entityTypeID = CCrmOwnerType::ResolveID($entityTypeName);
	if($entityTypeID === CCrmOwnerType::Undefined)
	{
		__CrmConstructionEditEndResponse(array('ERROR' => 'Undefined entity type is specified.'));
	}

	if($entityTypeID !== CCrmOwnerType::Construction)
	{
		__CrmConstructionEditEndResponse(array('ERROR' => "The '{$entityTypeName}' type is not supported in current context."));
	}

	if(!(CCrmConstruction::CheckCreatePermission($userPermissions) || CCrmConstruction::CheckUpdatePermission(0, $userPermissions)))
	{
		__CrmConstructionEditEndResponse(array('ERROR' => 'Access denied.'));
	}
	...
	$checker = new \Bitrix\Crm\Integrity\ConstructionDuplicateChecker();
	$groupResults = array();
	...
	foreach($groupData as &$group)
	{
		...
		$adapter = \Bitrix\Crm\EntityAdapterFactory::create($fields, CCrmOwnerType::Construction);
	...
	__CrmConstructionEditEndResponse(array('GROUP_RESULTS' => $groupResults));
}
elseif($action === 'FIND_LOCALITIES')
{
	...
	if($localityType === 'COUNTRY')
	{
		...
		__CrmConstructionEditEndResponse(array('DATA' => array('ITEMS' => $result)));
	}
	else
	{
		__CrmConstructionEditEndResponse(array('ERROR' => "Locality '{$localityType}' is not supported in current context."));
	}
}
else
{
	__CrmConstructionEditEndResponse(array('ERROR' => "Action '{$action}' is not supported in current context."));
}
