...
$arParams['PATH_TO_CONSTRUCTION_SHOW'] = CrmCheckPath('PATH_TO_CONSTRUCTION_SHOW', $arParams['PATH_TO_CONSTRUCTION_SHOW'], $APPLICATION->GetCurPage().'?construction_id=#construction_id#&show');
...
if($isEditMode)
{
	$isPermitted = CCrmConstruction::CheckUpdatePermission($arParams['ELEMENT_ID'], $userPermissions);
}
elseif($isCopyMode)
{
	$isPermitted = CCrmConstruction::CheckReadPermission($arParams['ELEMENT_ID'], $userPermissions);
}
else
{
	$isPermitted = CCrmConstruction::CheckCreatePermission($userPermissions);
}
...
$arEntityAttr = $arParams['ELEMENT_ID'] > 0
	? $userPermissions->GetEntityAttr('CONSTRUCTION', array($arParams['ELEMENT_ID']))
	: array();
...
if ($conversionWizard !== null && $_SERVER['REQUEST_METHOD'] === 'GET')
{
	...
	{
		$conversionWizard->prepareDataForEdit(CCrmOwnerType::Construction, $arFields, true);
	}
...
elseif ($isEditMode || $isCopyMode)
{
	...
	if ($isCopyMode)
	{
		$res = CCrmFieldMulti::GetList(
			array('ID' => 'asc'),
			array('ENTITY_ID' => 'CONSTRUCTION', 'ELEMENT_ID' => $arParams['ELEMENT_ID'])
		);
...
else
{
	if($_SERVER['REQUEST_METHOD'] == 'POST' && check_bitrix_sessid())
	{
			...
			if(isset($arResult['EXTERNAL_CONTEXT']) && $arResult['EXTERNAL_CONTEXT'] !== '')
			{
				$arResult['EXTERNAL_EVENT'] = array(
					...
					'PARAMS' => array(
						...
						'entityTypeName' => CCrmOwnerType::ConstructionName
					)
				);
			...
			else
			{
				LocalRedirect(
					isset($_REQUEST['backurl']) && $_REQUEST['backurl'] !== ''
						? $_REQUEST['backurl']
						: CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_CONSTRUCTION_LIST'], array())
				);
			}
		}
		elseif(isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']) || isset($_POST['continue']))
		{
			...
			elseif(!$isEditMode)
			{
				$arFields['OPENED'] = \Bitrix\Crm\Settings\ConstructionSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N';
			}
			...
			$USER_FIELD_MANAGER->EditFormAddFields(CCrmConstruction::$sUFEntityID, $arFields);
			if($conversionWizard !== null)
			{
				$conversionWizard->prepareDataForSave(CCrmOwnerType::Construction, $arFields);
			}
			elseif($isCopyMode)
			{
			...
			if (!$CCrmConstruction->CheckFields($arFields, $isEditMode ? $arResult['ELEMENT']['ID'] : false))
			{
				if (!empty($CCrmConstruction->LAST_ERROR))
					$arResult['ERROR_MESSAGE'] .= $CCrmConstruction->LAST_ERROR;
				else
			...
			if (empty($arResult['ERROR_MESSAGE']))
			{
				...
				if ($isEditMode)
				{
					$success = $CCrmConstruction->Update(
						$arResult['ELEMENT']['ID'],
						$arFields,
						true,
						true,
				...
				else
				{
					...
					$ID = $CCrmConstruction->Add($arFields, true, array('REGISTER_SONET_EVENT' => true));
					$success = $ID !== false;
					if($success)
					{
			...
			else
			{
				...
				{
					if (CCrmConstruction::CheckUpdatePermission($ID))
					{
						LocalRedirect(
							CComponentEngine::MakePathFromTemplate(
								$arParams['PATH_TO_CONSTRUCTION_EDIT'],
								array('construction_id' => $ID)
							)
						);
					}
				}
				...
				{
					if(CCrmConstruction::CheckReadPermission($ID))
					{
						LocalRedirect(
							CComponentEngine::MakePathFromTemplate(
	...
	else if (isset($_GET['delete']) && check_bitrix_sessid())
	{
		if ($isEditMode)
		{
			...
			if (!CCrmAuthorizationHelper::CheckDeletePermission(CCrmOwnerType::ConstructionName, $entityID, $userPermissions, $arEntityAttr))
			{
			...
			if (empty($arResult['ERROR_MESSAGE'])
				&& !$CCrmConstruction->Delete($arResult['ELEMENT']['ID'], array('PROCESS_BIZPROC' => false)))
			{
			...
			if (empty($arResult['ERROR_MESSAGE']))
				LocalRedirect(CComponentEngine::MakePathFromTemplate($arParams['PATH_TO_CONSTRUCTION_LIST']));
			else
			{
				ShowError($arResult['ERROR_MESSAGE']);
				return;
...
$arResult['FORM_ID'] = !empty($arParams['FORM_ID']) ? $arParams['FORM_ID'] : 'CRM_CONSTRUCTION_EDIT_V12';
$arResult['GRID_ID'] = 'CRM_CONSTRUCTION_LIST_V12';
$arResult['BACK_URL'] = $conversionWizard !== null && $conversionWizard->hasOriginUrl()
	? $conversionWizard->getOriginUrl() : $arParams['PATH_TO_CONSTRUCTION_LIST'];
...
$enableDupControl = $arResult['DUPLICATE_CONTROL']['ENABLED'] =
	!$isEditMode && \Bitrix\Crm\Integrity\DuplicateControl::isControlEnabledFor(CCrmOwnerType::Construction);

$arResult['FIELDS'] = array();
$arResult['FIELDS']['tab_1'][] = array(
	'id' => 'section_construction_info',
	'name' => GetMessage('CRM_SECTION_CONSTRUCTION_INFO2'),
	'type' => 'section'
);
...
$arResult['FIELDS']['tab_1'][] = array(
	'id' => 'BIRTHDATE',
	'name' => GetMessage('CRM_CONSTRUCTION_EDIT_FIELD_BIRTHDATE'),
	'type' => 'date_short',
	'value' => $birthDate
);
...
if (CCrmCompany::CheckReadPermission(0, $userPermissions))
{
	...
	{
		$companyBindings = \Bitrix\Crm\Binding\ConstructionCompanyTable::getConstructionBindings($arParams['ELEMENT_ID']);
	}
...
if($enableOutmodedFields)
{
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'type' => 'address',
		'componentParams' => array(
			'SERVICE_URL' => '/local/components/citfact/crm.construction.edit/ajax.php?siteID='.SITE_ID.'&'.bitrix_sessid_get(),
			'DATA' => array(
...
$arResult['FIELDS']['tab_1'][] = array(
	...
	'value' => isset($arResult['ELEMENT']['OPENED'])
		? $arResult['ELEMENT']['OPENED'] : (\Bitrix\Crm\Settings\ConstructionSettings::getCurrent()->getOpenedFlag() ? 'Y' : 'N'),
...
$GLOBALS['APPLICATION']->IncludeComponent('bitrix:crm.requisite.form.editor',
	...
	array(
		...
		'REQUISITE_ENTITY_TYPE_ID' => CCrmOwnerType::Construction,
...
$arResult['FIELDS']['tab_1'][] = array(
	'id' => 'ASSIGNED_BY_ID',
	'componentParams' => array(
		'NAME' => 'crm_construction_edit_resonsible',
...
else
{
	...
	$fileViewer = new \Bitrix\Crm\UserField\FileViewer(CCrmOwnerType::Construction, $arResult['ELEMENT']['ID']);
}
...
{
	CBPDocument::AddShowParameterInit('crm', 'only_users', 'CONSTRUCTION');
	...
	if (!isset($arDocumentStates))
	{
		$arDocumentStates = CBPDocument::GetDocumentStates(
			...
			$bEdit ? array('crm', 'CCrmDocumentConstruction', 'CONSTRUCTION_'.$arResult['ELEMENT']['ID']) : null
		);
	}
	...
	foreach ($arDocumentStates as $arDocumentState)
	{
		...
		$canViewWorkflow = CBPDocument::CanUserOperateDocument(
			...
			$USER->GetID(),
			array('crm', 'CCrmDocumentConstruction', $bEdit ? 'CONSTRUCTION_'.$arResult['ELEMENT']['ID'] : 'CONSTRUCTION_0'),
			array(
