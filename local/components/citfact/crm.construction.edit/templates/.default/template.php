...
$arResult['CRM_CUSTOM_PAGE_TITLE'] =
	$elementID > 0
	? GetMessage('CRM_CONSTRUCTION_EDIT_TITLE',
		array(
			'#ID#' => $elementID,
			'#NAME#' => CCrmConstruction::PrepareFormattedName(
				array(
	...
	: GetMessage('CRM_CONSTRUCTION_CREATE_TITLE');

$arFormButtons = array(
	'back_url' => $arResult['BACK_URL'],
	'standard_buttons' => true,
	'wizard_buttons' => false,
	'custom_html' => '<input type="hidden" name="construction_id" value="'.$elementID.'"/>'
);
...
<?
	$APPLICATION->IncludeComponent(
		...
		'',
		array(
			'PLACEMENT' => "CRM_CONSTRUCTION_EDIT",
			'PLACEMENT_APP' => 11,
...
?>
	...
	<script>
		BX.onCustomEvent('onCrmConstructionEditInterfaceInit', [{
			events: ['onConstructionFormChange'],
...
$APPLICATION->IncludeComponent(
	...
	array(
		...
		'IS_NEW' => $elementID <= 0,
		'USER_FIELD_ENTITY_ID' => CCrmConstruction::$sUFEntityID,
...
<script type="text/javascript">
	BX.ready(
		function()
		{
			...
			{
				title: "<?=GetMessageJS("CRM_CONSTRUCTION_EDIT_DUP_CTRL_SHORT_SUMMARY_TITLE")?>"
			};

			BX.CrmDuplicateWarningDialog.messages =
			{
				title: "<?=GetMessageJS("CRM_CONSTRUCTION_EDIT_DUP_CTRL_WARNING_DLG_TITLE")?>",
				acceptButtonTitle: "<?=GetMessageJS("CRM_CONSTRUCTION_EDIT_DUP_CTRL_WARNING_ACCEPT_BTN_TITLE")?>",
				cancelButtonTitle: "<?=GetMessageJS("CRM_CONSTRUCTION_EDIT_DUP_CTRL_WARNING_CANCEL_BTN_TITLE")?>"
			};

			BX.CrmEntityType.categoryCaptions =
			{
				"<?=CCrmOwnerType::LeadName?>": "<?=$entityTypeCategories[CCrmOwnerType::Lead]?>",
				"<?=CCrmOwnerType::ConstructionName?>": "<?=$entityTypeCategories[CCrmOwnerType::Construction]?>",
			...
			var dupControllerRequisite = BX.CrmDupControllerRequisite.create(
				(formID.toLowerCase() + "_dup_rq"),
				{
					...
					"groupSummaryTitle": "<?=GetMessageJS("CRM_CONSTRUCTION_EDIT_DUP_CTRL_REQUISITE_SUMMARY_TITLE")?>"
				}
			);
			var dupControllerBankDetail = BX.CrmDupControllerBankDetail.create(
				(formID.toLowerCase() + "_dup_bd"),
				{
					...
					"groupSummaryTitle": "<?=GetMessageJS("CRM_CONSTRUCTION_EDIT_DUP_CTRL_BANK_DETAIL_SUMMARY_TITLE")?>"
				}
			);
			var dupController = BX.CrmDupController.create(
				dupControllerId,
				{
					"serviceUrl": "/local/components/citfact/crm.construction.edit/ajax.php?&<?=bitrix_sessid_get()?>",
					...
					"groups":
					{
						...
						{
							"groupType": "fullName",
							"groupSummaryTitle": "<?=GetMessageJS("CRM_CONSTRUCTION_EDIT_DUP_CTRL_FULL_NAME_SUMMARY_TITLE")?>",
						...
						"email":
						{
							...
							"groupSummaryTitle": "<?=GetMessageJS("CRM_CONSTRUCTION_EDIT_DUP_CTRL_EMAIL_SUMMARY_TITLE")?>",
						...
						{
							"groupType": "communication",
							"groupSummaryTitle": "<?=GetMessageJS("CRM_CONSTRUCTION_EDIT_DUP_CTRL_PHONE_SUMMARY_TITLE")?>",
