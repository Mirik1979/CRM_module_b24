...
foreach($arResult['FIELDS']['tab_1'] as $field):
	...
	$field['id'] = "CONSTRUCTION_{$originFieldID}";
	if (strpos($field['id'], 'UF_') !== false):
		...
		if($field['id'] == 'CONSTRUCTION_COMPANY_ID' || $field['type'] == 'section')
			continue;
		elseif ($field['id'] == 'CONSTRUCTION_COMMENTS')
			$field['value'] = str_replace(":'COMMENTS'", ":'CONSTRUCTION_COMMENTS'", $field['value']);
		$arFields[] = $field;
	endif;
endforeach;
$arResult['FIELDS']['tab_1'] = array_merge($arFields, $arrAddRequiredFields, $arrAddFields);

$bChecked = !isset($_POST['CONVERT_CONSTRUCTION']) || $_POST['CONVERT_CONSTRUCTION'] == 'Y';
?>
...
<tr  style="background-color: #ededed;">
	<td class="bx-padding" colspan="2" style="background:none; text-align: center; font-weight: bold;">
			<input type="radio" value="Y" id="CONVERT_CONSTRUCTION_Y" name="CONVERT_CONSTRUCTION" <?=($bChecked ? 'checked' : '')?>
			onclick="var checked = this.checked; BX('CONSTRUCTION_form').style.display = checked ? '' : 'none'; BX('CONSTRUCTION_choise').style.display = !checked ? '' : 'none';" /><label for="CONVERT_CONSTRUCTION_Y" style="padding-right: 70px;"><?=GetMessage('CRM_FIELD_CONVERT_CONSTRUCTION')?></label>
			<input type="radio" value="N" id="CONVERT_CONSTRUCTION_N" name="CONVERT_CONSTRUCTION" <?=(!$bChecked ? 'checked' : '')?>
			onclick="var checked = this.checked; BX('CONSTRUCTION_form').style.display = !checked ? '' : 'none'; BX('CONSTRUCTION_choise').style.display = checked ? '' : 'none';"/><label for="CONVERT_CONSTRUCTION_N"><?=GetMessage('CRM_FIELD_CONVERT_CONSTRUCTION_LIST')?></label>
	</td>
</tr>
<tr id="CONSTRUCTION_choise" style="display:<?=(!$bChecked ? '' : 'none')?>">
	<td class="bx-field-name bx-padding" style="background:none"><span class="required">*</span><?=GetMessage('CRM_FIELD_CONVERT_CONSTRUCTION_CHOISE')?>:</td>
...
$GLOBALS["APPLICATION"]->IncludeComponent('bitrix:crm.entity.selector', 
	...
	array(
		...
		'INPUT_NAME' => 'CONSTRUCTION_CONSTRUCTION_ID',
		'INPUT_VALUE' => isset($arResult['ELEMENT']['CONSTRUCTION_ID']) ? $arResult['ELEMENT']['CONSTRUCTION_ID'] : '',
...
<div class="bx-edit-table" id="CONSTRUCTION_form" style="display:<?=($bChecked ? 'block' : 'none')?>">
<table cellpadding="0" cellspacing="0" border="0" class="bx-edit-table" id="tab_convert_construction_edit_table">
<?
$i = 0;
...
foreach($arResult['FIELDS']['tab_1'] as $field):
	...
	if ($sFirstAddField == $field["id"]):
		...
		?>
		<tr id="tr_construction_add_fields"><td colspan="2" class="bx-add-fields-section"><a href="javascript:showAdditionalFields('construction')" id="construction_a"><?=GetMessage('CRM_ADDITIONAL_FIELDS')?></a></td></tr>
		<?
	endif;		
...
<?
		...
		case 'intranet_user_search':
			...
			if(!empty($params)):
				$params['SEARCH_INPUT_NAME'] = 'CONSTRUCTION_'.$params['SEARCH_INPUT_NAME'];
				$params['INPUT_NAME'] = 'CONSTRUCTION_'.$params['INPUT_NAME'];
					...
					<td class="crm-offer-info-right">
						<div class="crm-offer-info-data-wrap"><?
							if(is_array($itemLocality)):
								$searchInputID = "CONSTRUCTION_{$arParams['FORM_ID']}_{$itemName}";
								$dataInputID = "CONSTRUCTION_{$arParams['FORM_ID']}_{$itemLocality['NAME']}";
								?><input class="crm-offer-item-inp" id="<?=$searchInputID?>" name="CONSTRUCTION_<?=$itemName?>" type="text" value="<?=htmlspecialcharsEx($itemValue)?>" />
								<input type="hidden" id="<?=$dataInputID?>" name="CONSTRUCTION_<?=$itemLocality['NAME']?>" value="<?=htmlspecialcharsbx($itemLocality['VALUE'])?>"/>
								...
								if(isset($item['IS_MULTILINE']) && $item['IS_MULTILINE']):
									?><textarea class="bx-crm-edit-text-area" name="CONSTRUCTION_<?=htmlspecialcharsEx($itemName)?>"><?=$itemValue?></textarea><?
								else:
									?><input class="crm-offer-item-inp" name="CONSTRUCTION_<?=htmlspecialcharsEx($itemName)?>" type="text" value="<?=htmlspecialcharsEx($itemValue)?>" /><?
								endif;
							endif;
						?></div>
					</td>
