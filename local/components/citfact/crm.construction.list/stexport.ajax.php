...
if(!function_exists('__CrmConstructionStExportEndResponse'))
{
	function __CrmConstructionStExportEndResponse($result)
	{
...
{
	__CrmConstructionStExportEndResponse(array('ERROR' => 'Site ID is not specified.'));
}


if (!CModule::IncludeModule('crm'))
{
	__CrmConstructionStExportEndResponse(array('ERROR' => 'Could not include crm module.'));
}
...
if ($action === 'STEXPORT')
{
	...
	if($entityTypeName === '')
	{
		__CrmConstructionStExportEndResponse(array('ERROR' => 'Entity type is not specified.'));
	}

	$exportType = isset($params['EXPORT_TYPE']) ? $params['EXPORT_TYPE'] : '';
	if(!in_array($exportType, array('csv', 'excel'), true))
	{
		__CrmConstructionStExportEndResponse(
	...
	{
		__CrmConstructionStExportEndResponse(array('ERROR' => 'Undefined entity type is specified.'));
	}
	...
	if(!CCrmPerms::IsAuthorized() || !CCrmConstruction::CheckExportPermission())
	{
		__CrmConstructionStExportEndResponse(array('ERROR' => 'Access denied.'));
	}
	...
	if($processToken === '')
	{
		__CrmConstructionStExportEndResponse(array('ERROR' => 'Process token is not specified.'));
	}

	$cParams = is_array($params['COMPONENT_PARAMS']) ? $params['COMPONENT_PARAMS'] : array();

	$progressData = CUserOptions::GetOption('crm', 'crm_stexport_construction', '');
	...
	if (!is_string($filePath) || strlen($filePath) === 0 || !CheckDirPath($filePath))
	{
		if (!$isNewToken)
		{
			CUserOptions::DeleteOption('crm', 'crm_stexport_construction');
			$processedItems = 0;
			$totalItems = 0;
			$blockSize = $defaultBlockSize;
		}
		...
		$fileName = "constructions.{$fileExt}";
		$tempDir = $_SESSION['CRM_EXPORT_TEMP_DIR'] =
			CTempFile::GetDirectoryName(1, array('crm', uniqid('construction_export_')));
		...
		CUserOptions::SetOption('crm', 'crm_stexport_construction', $progressData);
	}
	...
	do
	{
		...
		{
			__CrmConstructionStExportEndResponse(array('ERROR' => 'Progress data is incorrect.'));
		}
		...
		$cResult = $APPLICATION->IncludeComponent(
			'citfact:crm.construction.list',
			'',
			array(
				'CONSTRUCTION_COUNT' => $blockSize,
				'PATH_TO_CONSTRUCTION_LIST' => isset($cParams['PATH_TO_CONSTRUCTION_LIST']) ? $cParams['PATH_TO_CONSTRUCTION_LIST'] : '',
				'PATH_TO_CONSTRUCTION_SHOW' => isset($cParams['PATH_TO_CONSTRUCTION_SHOW']) ? $cParams['PATH_TO_CONSTRUCTION_SHOW'] : '',
				'PATH_TO_CONSTRUCTION_EDIT' => isset($cParams['PATH_TO_CONSTRUCTION_EDIT']) ? $cParams['PATH_TO_CONSTRUCTION_EDIT'] : '',
				...
				'NAVIGATION_CONTEXT_ID' => isset($cParams['NAVIGATION_CONTEXT_ID']) ?
					$cParams['NAVIGATION_CONTEXT_ID'] : 'CONSTRUCTION',
				'STEXPORT_MODE' => 'Y',
		...
		if (is_array($cResult))
		{
			...
			{
				__CrmConstructionStExportEndResponse(array('ERROR' => $cResult['ERROR']));
			}
			else
			{
	...
	if($processedItems < $totalItems && $processedItemsOnStep > 0 && $processedItemsOnStep >= $blockSize)
	{
		__CrmConstructionStExportEndResponse(
			array(
				...
				'TOTAL_ITEMS' => $totalItems,
				'SUMMARY' => GetMessage(
					'CRM_CONSTRUCTION_LIST_STEXPORT_PROGRESS_SUMMARY',
					array(
						'#PROCESSED_ITEMS#' => $processedItems,
						'#TOTAL_ITEMS#' => $totalItems
					)
	...
	{
		$fileUrl = '/local/components/bitrix/crm.construction.list/stexport.php?type='.$exportType;
		CUserOptions::DeleteOption('crm', 'crm_stexport_construction');
		__CrmConstructionStExportEndResponse(
			array(
				'STATUS' => 'COMPLETED',
				...
				'TOTAL_ITEMS' => $totalItems,
				'SUMMARY_HTML' => '<div>'.
					htmlspecialcharsbx(GetMessage('CRM_CONSTRUCTION_LIST_STEXPORT_COMPLETED_SUMMARY1')).'<br/>'.
					htmlspecialcharsbx(
						GetMessage(
							'CRM_CONSTRUCTION_LIST_STEXPORT_COMPLETED_SUMMARY2',
							array('#PROCESSED_ITEMS#' => $processedItems)
						)
					).'<br/><br/></div><div><a href="'.htmlspecialcharsbx($fileUrl).'">'.
					htmlspecialcharsbx(GetMessage('CRM_CONSTRUCTION_LIST_STEXPORT_DOWNLOAD')).'</a></div>'
			)
		);
	}
}
