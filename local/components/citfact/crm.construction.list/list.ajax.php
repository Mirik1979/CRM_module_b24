...
if(!function_exists('__CrmConstructionListEndResponse'))
{
	function __CrmConstructionListEndResponse($result)
	{
...
{
	__CrmConstructionListEndResponse(array('ERROR' => 'Could not include crm module.'));
}
...
{
	__CrmConstructionListEndResponse(array('ERROR' => 'Access denied.'));
}
...
if ($_REQUEST['MODE'] == 'SEARCH')
{
	...
	if(!CCrmConstruction::CheckReadPermission(0, $userPerms))
	{
		__CrmConstructionListEndResponse(array('ERROR' => 'Access denied.'));
	}
	...
	if (!empty($search))
	{
		...
		$arConstructionTypeList = CCrmStatus::GetStatusListEx('CONSTRUCTION_TYPE');
		$arSelect = array('ID', 'HONORIFIC', 'NAME', 'SECOND_NAME', 'LAST_NAME', 'COMPANY_TITLE', 'PHOTO', 'TYPE_ID');
		$arOrder = array('LAST_NAME' => 'ASC', 'NAME' => 'ASC');
		$obRes = CCrmConstruction::GetListEx($arOrder, $arFilter, false, array('nTopCount' => $nPageTop), $arSelect);
		$arImages = array();
		$arLargeImages = array();

		$i = 0;
		$constructionIndex = array();
		$constructionTypes = CCrmStatus::GetStatusList('CONSTRUCTION_TYPE');
		while ($arRes = $obRes->Fetch())
		{
			...
			// advanced info
			$advancedInfo = array();
			if (isset($arRes['TYPE_ID']) && $arRes['TYPE_ID'] != '' && isset($constructionTypes[$arRes['TYPE_ID']]))
			{
				$advancedInfo['constructionType'] = array(
					'id' => $arRes['TYPE_ID'],
					'name' => $constructionTypes[$arRes['TYPE_ID']]
				);
			}
			...
			$arData[$i] =
				array(
					...
					'url' => CComponentEngine::MakePathFromTemplate(COption::GetOptionString('crm', 'path_to_construction_show'),
						array(
							'construction_id' => $arRes['ID']
						)
					),
			...
			if ($requireRequisiteData)
				$arData[$i]['advancedInfo']['requisiteData'] = CCrmEntitySelectorHelper::PrepareRequisiteData(
					CCrmOwnerType::Construction, $arRes['ID'], array('VIEW_DATA_ONLY' => true)
				);

			$constructionIndex[$arRes['ID']] = &$arData[$i];
			$i++;
		}

		// advanced info - phone number, e-mail
		$obRes = CCrmFieldMulti::GetList(array('ID' => 'asc'), array('ENTITY_ID' => 'CONSTRUCTION', 'ELEMENT_ID' => array_keys($constructionIndex)));
		while($arRes = $obRes->Fetch())
		{
			if (isset($constructionIndex[$arRes['ELEMENT_ID']])
				&& ($arRes['TYPE_ID'] === 'PHONE' || $arRes['TYPE_ID'] === 'EMAIL'))
			{
				$item = &$constructionIndex[$arRes['ELEMENT_ID']];
		...
		unset($constructionIndex);
	}

	__CrmConstructionListEndResponse($arData);
}
elseif ($action === 'REBUILD_SEARCH_CONTENT')
{
	$agent = \Bitrix\Crm\Agent\Search\ConstructionSearchContentRebuildAgent::getInstance();
	...
	if(!$agent->isEnabled())
	{
		__CrmConstructionListEndResponse(array('STATUS' => 'COMPLETED'));
	}

	$progressData = $agent->getProgressData();
	__CrmConstructionListEndResponse(
		array(
			'STATUS' => 'PROGRESS',
...
{
	$agent = \Bitrix\Crm\Agent\Timeline\ConstructionTimelineBuildAgent::getInstance();
	if($agent->isEnabled() && !$agent->isActive())
	{
		$agent->enable(false);
	}
...
elseif ($action === 'REBUILD_DUPLICATE_INDEX')
{
	...
	if($entityTypeName === '')
	{
		__CrmConstructionListEndResponse(array('ERROR' => 'Entity type is not specified.'));
	}

	$entityTypeID = CCrmOwnerType::ResolveID($entityTypeName);
	if($entityTypeID === CCrmOwnerType::Undefined)
	{
		__CrmConstructionListEndResponse(array('ERROR' => 'Undefined entity type is specified.'));
	}

	if($entityTypeID !== CCrmOwnerType::Construction)
	{
		__CrmConstructionListEndResponse(array('ERROR' => "The '{$entityTypeName}' type is not supported in current context."));
	}

	if(!CCrmConstruction::CheckUpdatePermission(0))
	{
		__CrmConstructionListEndResponse(array('ERROR' => 'Access denied.'));
	}
	...
	if(COption::GetOptionString('crm', '~CRM_REBUILD_CONSTRUCTION_DUP_INDEX', 'N') !== 'Y')
	{
		__CrmConstructionListEndResponse(
			array(
				'STATUS' => 'NOT_REQUIRED',
				'SUMMARY' => GetMessage('CRM_CONSTRUCTION_LIST_REBUILD_DUPLICATE_INDEX_NOT_REQUIRED_SUMMARY')
			)
		);
	}

	$progressData = COption::GetOptionString('crm', '~CRM_REBUILD_CONSTRUCTION_DUP_INDEX_PROGRESS',  '');
	...
	if($totalItemQty <= 0)
	{
		$totalItemQty = CCrmConstruction::GetListEx(array(), array('CHECK_PERMISSIONS' => 'N'), array(), false);
	}
	...
	if($itemQty > 0)
	{
		CCrmConstruction::RebuildDuplicateIndex($itemIDs);
		...
		COption::SetOptionString('crm', '~CRM_REBUILD_CONSTRUCTION_DUP_INDEX_PROGRESS', serialize($progressData));
		__CrmConstructionListEndResponse(
			array(
				'STATUS' => 'PROGRESS',
				...
				'SUMMARY' => GetMessage(
					'CRM_CONSTRUCTION_LIST_REBUILD_DUPLICATE_INDEX_PROGRESS_SUMMARY',
					array(
						'#PROCESSED_ITEMS#' => $processedItemQty,
						'#TOTAL_ITEMS#' => $totalItemQty
					)
	...
	{
		COption::RemoveOption('crm', '~CRM_REBUILD_CONSTRUCTION_DUP_INDEX');
		COption::RemoveOption('crm', '~CRM_REBUILD_CONSTRUCTION_DUP_INDEX_PROGRESS');
		__CrmConstructionListEndResponse(
			array(
				'STATUS' => 'COMPLETED',
				'PROCESSED_ITEMS' => $processedItemQty,
				'TOTAL_ITEMS' => $totalItemQty,
				'SUMMARY' => GetMessage(
					'CRM_CONSTRUCTION_LIST_REBUILD_DUPLICATE_INDEX_COMPLETED_SUMMARY',
...
{
	//~CRM_REBUILD_CONSTRUCTION_ACT_STATISTICS
	\Bitrix\Main\Localization\Loc::loadMessages(__FILE__);
	...
	if(COption::GetOptionString('crm', '~CRM_REBUILD_CONSTRUCTION_ACT_STATISTICS', 'N') !== 'Y')
	{
		__CrmConstructionListEndResponse(
			array(
				'STATUS' => 'NOT_REQUIRED',
				'SUMMARY' => GetMessage('CRM_CONSTRUCTION_LIST_REBUILD_ACT_STATISTICS_NOT_REQUIRED_SUMMARY')
			)
		);
	}

	$progressData = COption::GetOptionString('crm', '~CRM_REBUILD_CONSTRUCTION_ACT_STATISTICS_PROGRESS',  '');
	$progressData = $progressData !== '' ? unserialize($progressData) : array();
	...
	if($itemQty > 0)
	{
		Crm\Activity\CommunicationStatistics::rebuild(\CCrmOwnerType::Construction, $itemIDs);
		...
		COption::SetOptionString('crm', '~CRM_REBUILD_CONSTRUCTION_ACT_STATISTICS_PROGRESS', serialize($progressData));
		__CrmConstructionListEndResponse(
			array(
				'STATUS' => 'PROGRESS',
				'PROCESSED_ITEMS' => $processedItemQty,
				'TOTAL_ITEMS' => $totalItemQty,
				'SUMMARY' => GetMessage(
					'CRM_CONSTRUCTION_LIST_REBUILD_ACT_STATISTICS_PROGRESS_SUMMARY',
					array(
						'#PROCESSED_ITEMS#' => $processedItemQty,
						'#TOTAL_ITEMS#' => $totalItemQty
					)
	...
	{
		COption::RemoveOption('crm', '~CRM_REBUILD_CONSTRUCTION_ACT_STATISTICS');
		COption::RemoveOption('crm', '~CRM_REBUILD_CONSTRUCTION_ACT_STATISTICS_PROGRESS');
		__CrmConstructionListEndResponse(
			array(
				'STATUS' => 'COMPLETED',
				'PROCESSED_ITEMS' => $processedItemQty,
				'TOTAL_ITEMS' => $totalItemQty,
				'SUMMARY' => GetMessage(
					'CRM_CONSTRUCTION_LIST_REBUILD_ACT_STATISTICS_COMPLETED_SUMMARY',
					array('#PROCESSED_ITEMS#' => $processedItemQty)
				)
			)
		);
...
elseif ($action === 'GET_ROW_COUNT')
{
	...
	{
		__CrmConstructionListEndResponse(array('DATA' => array('TEXT' => '')));
	}

	$gridData = $_SESSION['CRM_GRID_DATA'][$gridID];
	$filter = isset($gridData['FILTER']) && is_array($gridData['FILTER']) ? $gridData['FILTER'] : array();
	$result = CCrmConstruction::GetListEx(array(), $filter, array(), false, array(), array());

	$text = '';
	if(is_numeric($result))
	{
		$text = GetMessage('CRM_CONSTRUCTION_LIST_ROW_COUNT', array('#ROW_COUNT#' => $result));
		if($text === '')
		{
			$text = $result;
		}
	}
	__CrmConstructionListEndResponse(array('DATA' => array('TEXT' => $text)));
}
elseif ($action === 'GET_REQUISITE_PRESETS')
{
	...
	if(!CCrmConstruction::CheckReadPermission(0))
	{
		__CrmConstructionListEndResponse(array('ERROR' => 'Access denied.'));
	}
...
elseif ($action === 'BUILD_REQUISITES')
{
	...
	if($presetID <= 0)
	{
		__CrmConstructionListEndResponse(array('ERROR' => "Preset ID is not specified."));
	}

	if(!(CCrmConstruction::CheckReadPermission(0) && CCrmConstruction::CheckUpdatePermission(0)))
	{
		__CrmConstructionListEndResponse(array('ERROR' => 'Access denied.'));
	}

	$progressData = COption::GetOptionString('crm', '~CRM_CONSTRUCTION_REQUISITES_BUILD_PROGRESS',  '');
	$progressData = $progressData !== '' ? unserialize($progressData) : array();
	$lastItemID = isset($progressData['LAST_ITEM_ID']) ? intval($progressData['LAST_ITEM_ID']) : 0;
	$processedItemQty = isset($progressData['PROCESSED_ITEMS']) ? intval($progressData['PROCESSED_ITEMS']) : 0;
	$totalItemQty = isset($progressData['TOTAL_ITEMS']) ? intval($progressData['TOTAL_ITEMS']) : 0;
	...
	if($itemQty > 0)
	{
		...
		{
			CCrmConstruction::CreateRequisite($itemID, $presetID);
		}
		...
		COption::SetOptionString('crm', '~CRM_CONSTRUCTION_REQUISITES_BUILD_PROGRESS', serialize($progressData));
		__CrmConstructionListEndResponse(
			array(
				'STATUS' => 'PROGRESS',
				'PROCESSED_ITEMS' => $processedItemQty,
				'TOTAL_ITEMS' => $totalItemQty,
				'SUMMARY' => GetMessage(
					'CRM_CONSTRUCTION_REQUISITES_BUILD_PROGRESS_SUMMARY',
					array(
						'#PROCESSED_ITEMS#' => $processedItemQty,
						'#TOTAL_ITEMS#' => $totalItemQty
					)
	...
	{
		COption::RemoveOption('crm', '~CRM_CONSTRUCTION_REQUISITES_BUILD_PROGRESS');
		__CrmConstructionListEndResponse(
			array(
				'STATUS' => 'COMPLETED',
				'PROCESSED_ITEMS' => $processedItemQty,
				'TOTAL_ITEMS' => $totalItemQty,
				'SUMMARY' => GetMessage(
					'CRM_CONSTRUCTION_REQUISITES_BUILD_COMPLETED_SUMMARY',
					array('#PROCESSED_ITEMS#' => $processedItemQty)
				)
			)
		);
...
{
	COption::RemoveOption('crm', '~CRM_TRANSFER_REQUISITES_TO_CONSTRUCTION');
}
elseif ($action === 'CONVERT_REQUISITES')
{
	...
	if(COption::GetOptionString('crm', '~CRM_TRANSFER_REQUISITES_TO_CONSTRUCTION', 'N') !== 'Y')
	{
		__CrmConstructionListEndResponse(
			array(
				'STATUS' => 'NOT_REQUIRED',
				'SUMMARY' => GetMessage('CRM_CONSTRUCTION_REQUISITES_TRANSFER_NOT_REQUIRED_SUMMARY')
			)
		);
	}

	$progressData = COption::GetOptionString('crm', '~CRM_CONSTRUCTION_REQUISITES_TRANSFER_PROGRESS',  '');
	$progressData = $progressData !== '' ? unserialize($progressData) : array();
	$lastItemID = isset($progressData['LAST_ITEM_ID']) ? (int)($progressData['LAST_ITEM_ID']) : 0;
	$processedItemQty = isset($progressData['PROCESSED_ITEMS']) ? (int)($progressData['PROCESSED_ITEMS']) : 0;
	$totalItemQty = isset($progressData['TOTAL_ITEMS']) ? (int)($progressData['TOTAL_ITEMS']) : 0;
	...
	$converters = array(
		new Crm\Requisite\InvoiceRequisiteConverter(CCrmOwnerType::Construction, $presetID, true, 300),
		new Crm\Requisite\AddressRequisiteConverter(CCrmOwnerType::Construction, $presetID, true)
	);
	...
	if($itemQty > 0)
	{
		foreach($converters as $converter)
		{
			...
			{
				__CrmConstructionListEndResponse(array('ERROR' => $e->getLocalizedMessage()));
			}
			catch(Exception $e)
			{
				__CrmConstructionListEndResponse(array('ERROR' => $e->getMessage()));
			}
		}
		...
		COption::SetOptionString('crm', '~CRM_CONSTRUCTION_REQUISITES_TRANSFER_PROGRESS', serialize($progressData));
		__CrmConstructionListEndResponse(
			array(
				'STATUS' => 'PROGRESS',
				'PROCESSED_ITEMS' => $processedItemQty,
				'TOTAL_ITEMS' => $totalItemQty,
				'SUMMARY' => GetMessage(
					'CRM_CONSTRUCTION_REQUISITES_TRANSFER_PROGRESS_SUMMARY',
					array(
						'#PROCESSED_ITEMS#' => $processedItemQty,
						'#TOTAL_ITEMS#' => $totalItemQty
					)
	...
	else
	{
		...
		COption::RemoveOption('crm', '~CRM_CONSTRUCTION_REQUISITES_TRANSFER_PROGRESS');
		COption::RemoveOption('crm', '~CRM_TRANSFER_REQUISITES_TO_CONSTRUCTION');
		__CrmConstructionListEndResponse(
			array(
				'STATUS' => 'COMPLETED',
				...
				'TOTAL_ITEMS' => $totalItemQty,
				'SUMMARY' => GetMessage(
					'CRM_CONSTRUCTION_REQUISITES_TRANSFER_COMPLETED_SUMMARY',
					array('#PROCESSED_ITEMS#' => $processedItemQty)
				)
			)
		);
