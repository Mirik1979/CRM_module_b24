...
if(CModule::IncludeModule('crm'))
{
	...
	if($exportType === 'csv' || $exportType === 'excel')
	{
		...
		$exportFileName = "constructions.{$exportFileExt}";
		...
		if($exportFilePath !== '' && file_exists($exportFilePath))
		{
			...
			if(is_resource($file))
			{
				...
				else
				{
					...
					$errMsg = GetMessage('CRM_CONSTRUCTION_LIST_STEXPORT_ERR_GET_FILE_SIZE').PHP_EOL;
					$errorOccured = true;
				}
			}
		}
		...
		{
			\Bitrix\Main\Localization\Loc::loadMessages(__FILE__);
			$errMsg = GetMessage('CRM_CONSTRUCTION_LIST_STEXPORT_ERR_FILE_NOT_FOUND').PHP_EOL;
			$errorOccured = true;
		}
	}
	else
	{
		\Bitrix\Main\Localization\Loc::loadMessages(__FILE__);
		$errMsg = GetMessage('CRM_CONSTRUCTION_LIST_STEXPORT_ERR_INVALID_TYPE').PHP_EOL;
		$errorOccured = true;
	}
