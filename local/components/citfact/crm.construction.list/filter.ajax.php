...
elseif(!(\CCrmConstruction::CheckReadPermission() && check_bitrix_sessid()))
{
...
else
{
	...
	$enableOutmodedFields = Crm\Settings\ConstructionSettings::getCurrent()->areOutmodedRequisitesEnabled();
	...
	$filter = \Bitrix\Crm\Filter\Factory::createEntityFilter(
		new \Bitrix\Crm\Filter\ConstructionSettings(
			array(
				'ID' => isset($_REQUEST['filter_id']) ? $_REQUEST['filter_id'] : 'CRM_CONSTRUCTION_LIST_V12',
				'flags' => $filterFlags
			)
		)
	);
