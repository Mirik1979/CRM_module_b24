...
class CStoreDetailsComponent extends CBitrixComponent
{
	/** @var string */
	protected $guid = '';
	/** @var int */
	...
	public function executeComponent()
	{
        ...
        $this->arResult['CONTEXT_ID'] = 'STORE_'.$this->arResult['ENTITY_ID'];
        ...
        // история с ней отдельно разбираемся
		...
		// можно комментить - берет номер телефона
		...
		if($this->entityID > 0)
		{
			// проверка существования компнии
		...
		// режим редактирования и копирования
        ...
        // конвертация из лида - комментирую
        /*
		...
		//region GUID
        // выбор GUID - можно наверное оставить как есь
		$this->guid = $this->arResult['GUID'] = isset($this->arParams['GUID'])
			? $this->arParams['GUID']
			: ($this->isMyCompany() ? "my_store_{$this->entityID}_details" : "stores_{$this->entityID}_details");
        $this->arResult['EDITOR_CONFIG_ID'] = isset($this->arParams['EDITOR_CONFIG_ID'])
        	? $this->arParams['EDITOR_CONFIG_ID'] : 'store_details';
		//$this->arResult['EDITOR_CONFIG_ID'] = isset($this->arParams['EDITOR_CONFIG_ID'])
		...
		$this->arResult['ENTITY_INFO'] = array(
            ...
            // странный ID
            //'ENTITY_TYPE_ID' => 4,
            'ENTITY_TYPE_NAME' => 'STORES',
			'TITLE' => isset($this->entityData['NAME']) ? $this->entityData['NAME'] : '',
			// details
			//'SHOW_URL' => CCrmOwnerType::GetEntityShowPath(CCrmOwnerType::Company, $this->entityID, false),
            'SHOW_URL' => '/crm/stores/details/'.$this->entityID.'/'
		);
		//endregion
		...
		if($this->isCopyMode)
		{
			$APPLICATION->SetTitle(Loc::getMessage('CRMSTORES_COPY_PAGE_TITLE'));
		}
		elseif(isset($this->entityData['NAME']))
		{
			$APPLICATION->SetTitle(htmlspecialcharsbx($this->entityData['NAME']));
		}
		elseif(!$this->isEditMode)
		{
			$APPLICATION->SetTitle(Loc::getMessage('CRMSTORES_CREATION_PAGE_TITLE'));
		}
		//endregion
		...
		// распределение полей по секциям - переписываем
		$this->arResult['ENTITY_CONFIG'] = array(
			array(
				'name' => 'main',
				'title' => Loc::getMessage('CRMSTORES_SECTION_MAIN'),
				'type' => 'section',
				'elements' =>
					array_merge(
						array(
			...
			array(
				'name' => 'additional',
				'title' => Loc::getMessage('CRMSTORES_SECTION_ADDITIONAL'),
				'type' => 'section',
				'elements' =>
					array_merge(
						array(
		...
		//region TABS
		// дополнительные закладки - собираем аяксами - надо отдельно исследовать
		...
		if($this->entityID > 0)
		{
			if($this->isMyCompany())
			{
				...
				/* пока тоже комментируем - после будем разбираться
		...
		// сценарий с пустым ID - на создание
		else
		{
			$this->arResult['TABS'][] = array(
				'id' => 'tab_deal',
	...
	public function prepareFieldInfos()
	{
        ...
        // переписываем под поля в сделке
		/*$this->arResult['ENTITY_FIELDS'] = array(
			array(
				'name' => 'ID',
...
//			),
		...
		// старые адреса - нафиг не нужны, комментируем
		...
		// телефоны и прочие - нафиг не нужны
	...
	public function prepareEntityData()
	{
		...
		//region User Fields
        // наполнение пользовательских полей
