...
$arResult['HEADERS'] = array(
    array('id' => 'ID', 'name' => GetMessage('CRM_COLUMN_ID')),
    array('id' => 'NAME', 'name' => GetMessage('CRM_COLUMN_TITLE')),
    array('id' => 'ASSIGNED_BY_ID', 'name' => GetMessage('CRM_COLUMN_ASSIGNED_BY_ID'))
);
...
else if (isset($_REQUEST['import']) && isset($_SESSION['CRM_IMPORT_FILE']))
{
	...
	while($arData = $csvFile->Fetch())
	{
				...
				|| $enableDupCtrlByEmail || $enableDupCtrlByRequisite))
		{
			...
			unset($dup);
            // найдены дубликаты - обсудить по каким полям искать
			if(!empty($dupIDs))
			{
				$isDuplicate = true;
		...
		else
		{
            ...
            // добавление в свою строку
            // тут проверяем что в массиве
            $entity = new StoreTable(false);
            $id = $entity->Add($arCompany);
