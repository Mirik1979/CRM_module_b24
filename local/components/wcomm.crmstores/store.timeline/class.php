...
class CCrmTimelineComponent extends CBitrixComponent
{
	...
	public function prepareScheduleItems()
	{
		...
		$dbResult = \local\Crm\Activity\CCrmActivityAltLocal::GetList(
			array('DEADLINE' => 'ASC'),
			$filter,
			false,
			false,
		...
		while($fields = $dbResult->Fetch())
		{
			...
			if(isset($tmpData["ASSOCIATED_ENTITY"]["PROVIDER_TYPE_ID"])
				&& (($tmpData["ASSOCIATED_ENTITY"]["PROVIDER_TYPE_ID"] == "CALL") 
					|| ($tmpData["ASSOCIATED_ENTITY"]["PROVIDER_TYPE_ID"] == "MEETING"))
				&& ($tmpData["ASSOCIATED_ENTITY"]["PERMISSIONS"]["COMPLETE"] == true))
			{
	...
	public function loadHistoryItems($offsetTime, &$nextOffsetTime, $offsetID, &$nextOffsetID, $limit, $onlyFixed = false)
	{
		...
		if($this->entityTypeID==3 && $this->entityID>0){
            ...
            if(count($id)>0){
                ...
                $query->registerRuntimeField("BIND", new ReferenceField('BIND',
                ...
                $query->registerRuntimeField('IS_FIXED',new Entity\ExpressionField('IS_FIXED', '%s', 'BIND.IS_FIXED'));

                $query->registerRuntimeField('BIND_ENTITY_TYPE_ID',new Entity\ExpressionField('BIND_ENTITY_TYPE_ID', '%s', 'BIND.ENTITY_TYPE_ID'));
                $query->registerRuntimeField('BIND_ENTITY_ID',new Entity\ExpressionField('BIND_ENTITY_ID', '%s', 'BIND.ENTITY_ID'));

                $query->addFilter(null,[
                    'LOGIC'=>'OR',
                    ['=BIND_ENTITY_TYPE_ID', $this->entityTypeID, '=BIND_ENTITY_ID', $this->entityID],
                    ['ASSOCIATED_ENTITY_ID', $id],
                ]);

                if ($onlyFixed)
                {
                    $query->addFilter('=IS_FIXED', 'Y');
                }

                $query->addSelect('IS_FIXED');
