...
else
{
    ...
    //\Bitrix\Main\Diag\Debug::writeToFile("match1", "match1", "__miros.log");
    //\Bitrix\Main\Diag\Debug::writeToFile($selectedTypes, "match1 types", "__miros.log");
    //\Bitrix\Main\Diag\Debug::writeToFile($entityTypeID, "match1 types", "__miros.log");
    //\Bitrix\Main\Diag\Debug::writeToFile($userID, "match1 user", "__miros.log");
    //\Bitrix\Main\Diag\Debug::writeToFile($enablePermissionCheck, "match1 check", "__miros.log");

    $entityTypeID = 6;

    $list = new Integrity\DuplicateList(
    //$list = new \local\DuplicateListPlus(
		Integrity\DuplicateIndexType::joinType(array_keys($selectedTypes)),
		$entityTypeID,
		$userID,
		$enablePermissionCheck
	);
    //\Bitrix\Main\Diag\Debug::writeToFile($currentScope, "match1 scope", "__miros.log");
    //\Bitrix\Main\Diag\Debug::writeToFile($sortTypeID, "match1 type", "__miros.log");
    //\Bitrix\Main\Diag\Debug::writeToFile($sortOrder, "match1 sort", "__miros.log");
    ...
    foreach($items as $item) {
        ...
        $value = $USER_FIELD_MANAGER->GetUserFieldValue('CRM_STORES', 'UF_CRM_1572510722', $entityID);
        //\Bitrix\Main\Diag\Debug::writeToFile('idddd', "idddd", "__miros.log");
        //\Bitrix\Main\Diag\Debug::writeToFile($entityID, "idddd", "__miros.log");
        if (!Loader::includeModule('wcomm.crmstores')) {
            ShowError(Loc::getMessage('CRMSTORES_NO_MODULE'));
            return;
        }
        $addid = StoreTable::getbyId($entityID);
        $newadd = $addid->fetchAll();
        //\Bitrix\Main\Diag\Debug::writeToFile('idddd', "idddd", "__miros.log");
        $dbUsers = UserTable::getList(array(
            'filter' => array('=ID' => $newadd[0]['ASSIGNED_BY_ID'])
        ));

        while($fields = $dbUsers ->Fetch())
        {
            $assignname = $fields['NAME']." ".$fields['LAST_NAME'];
            //print_r($fields);
            //echo "</pre>";
        }
        ...
        $entityInfos[$entityID] = array(
            'TITLE' => $newadd[0]['NAME'],
            'LEGEND' => '',
            'RESPONSIBLE_ID' => $newadd[0]['ASSIGNED_BY_ID'],
            'SHOW_URL' => '/crm/stores/details/'.$newadd[0]['ID'].'/',
            'RESPONSIBLE_FULL_NAME' => $assignname,
            'PHONE' =>  array(
                'FIRST_VALUE' => $value,
                'TOTAL' => 1
            )
        );
        //$arResult['ENTITY_INFOS'][$entityID] = "xxx";
    }
