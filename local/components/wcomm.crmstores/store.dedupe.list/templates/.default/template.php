		...
		function()
		{
			...
			var list = BX.CrmDedupeList.create(
				"<?=CUtil::JSEscape($listID)?>",
				{
					...
					// выпадающий список в аяксе
                    serviceUrl: "<?='/local/components/wcomm.crmstores/store.dedupe.list/ajax.php?'.bitrix_sessid_get()?>",
		...
		function()
		{
			...
			var mgr = BX.CrmDuplicateManager.create(
				"mgr",
				{
					...
					//serviceUrl: "<?=SITE_DIR?>local/components/wcomm.crmstores/store.dedupe.list/list.ajax.php?&<?=bitrix_sessid_get()?>"
				}
			);
			BX.addCustomEvent(
				mgr,
