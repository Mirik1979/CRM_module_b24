...
if (is_array($arResult['VALUE']) && count($arResult['VALUE']) > 0)
{
    ...
    $arResult['VALUE'] = unserialize($arResult['~VALUE'][0]);
	...
	{
		if (is_numeric($value[$arParams['ENTITY_TYPE'][0]]))
			$arValue[$arParams['ENTITY_TYPE'][0]][] = $value[$arParams['ENTITY_TYPE'][0]];
		else
		{
			$ar = explode('_', $value[$arParams['ENTITY_TYPE'][0]]);
	...
	&& isset($arValue['COMPANY']) && !empty($arValue['COMPANY']))
	{
		...
		while ($arRes = $dbRes->Fetch())
		{
            $result['COMPANY'][$arRes['ID']] = Array(
				'ENTITY_TITLE' => $arRes['TITLE'],
				'ENTITY_LINK' => CComponentEngine::MakePathFromTemplate(COption::GetOptionString('crm', 'path_to_company_details'), array('company_id' => $arRes['ID']))
			);
		}
	}
	if ($arParams['arUserField']['SETTINGS']['DEAL'] == 'Y'
    ...
    foreach ($arResult['VALUE'] as $value)
    {
        $newArResultValue[] = array_merge(
            $result[$arParams['ENTITY_TYPE'][0]][$value[$arParams['ENTITY_TYPE'][0]]],
            ['ID' => $value[$arParams['ENTITY_TYPE'][0]], 'ENTITY_TYPE' => $ar[$value['TYPE_'.$arParams['ENTITY_TYPE'][0]]]]
        );
    }

    $arResult['VALUE'] = [];
    $arResult['VALUE'][$arParams['ENTITY_TYPE'][0]] = $newArResultValue;
}
